...
class BrickPredictor:
    ...
    def _load_model(self):
        ...
        if self.model_type == 'lightgbm':
            ...
            if model_file.exists():
                ...
                print(f"LightGBM模型加载成功: {model_file}")
            else:
                print(f"LightGBM模型文件不存在: {model_file}")
        else:
            model_file = OUTPUT_DIR / "xgboost_brick_model.json"
            config_file = OUTPUT_DIR / "xgboost_config.json"
            if model_file.exists():
                self.model = xgb.Booster()
                self.model.load_model(str(model_file))
                print(f"XGBoost模型加载成功: {model_file}")
            else:
                print(f"XGBoost模型文件不存在: {model_file}")
    ...
    def predict_batch(self, data_dir: Path, top_n: int = 10) -> List[Dict]:
        results = []
...
def main():
    predictor = BrickPredictor(model_type='xgboost')
    data_dir = Path("/Users/lidongyang/Desktop/Qstrategy/data/20260315")
    results = predictor.predict_batch(data_dir, top_n=20)
    print("\n" + "="*80)
    print("砖型策略ML预测结果 (Top 20)")
    print("="*80)
    print(f"{'代码':<8} {'名称':<10} {'日期':<12} {'收盘价':<10} {'预测概率':<10} {'预测结果'}")
    print("-"*80)
    for r in results:
        pred_label = "正样本" if r['prediction'] == 1 else "负样本"
        print(f"{r['code']:<8} {r['name']:<10} {str(r['date'])[:10]:<12} {r['close']:<10.2f} {r['proba']:<10.4f} {pred_label}")
    output_file = OUTPUT_DIR / "ml_predictions.csv"
