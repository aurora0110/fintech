...
TRAIN_DATA_FILE = Path("/Users/lidongyang/Desktop/Qstrategy/ml/brick_train_dataset.csv")
OUTPUT_DIR = Path("/Users/lidongyang/Desktop/Qstrategy/ml")

HOLD_DAYS = 3
POSITIVE_RETURN_THRESHOLD = 0.035
STOP_LOSS_RATIO = 0.99
TEST_SAMPLES_TARGET = 200
...
def build_test_dataset(train_dates: set):
    print("构建独立测试集...")
    ...
    for file_path in all_files:
        if len(positive_samples) >= TEST_SAMPLES_TARGET and len(negative_samples) >= TEST_SAMPLES_TARGET:
            break
        ...
        for i in range(20, len(df) - HOLD_DAYS - 5):
            ...
            if check_positive_label(df, i) and len(positive_samples) < TEST_SAMPLES_TARGET:
                features['label'] = 1
                positive_samples.append(features)
            elif check_negative_label(df, i) and len(negative_samples) < TEST_SAMPLES_TARGET:
                features['label'] = 0
...
def main():
    ...
    print(f"训练集日期数: {len(train_dates)}")
    
    test_df = build_test_dataset(train_dates)
    if test_df is None:
        return
    ...
    lgb_model_file = OUTPUT_DIR / "lightgbm_brick_model.txt"
    if lgb_model_file.exists():
        ...
        print(f"LightGBM模型文件不存在: {lgb_model_file}")
    
    xgb_model_file = OUTPUT_DIR / "xgboost_brick_model.json"
    if xgb_model_file.exists():
        ...
        print(f"XGBoost模型文件不存在: {xgb_model_file}")
