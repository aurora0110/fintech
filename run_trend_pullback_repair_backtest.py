...
@dataclass
class RepairConfig:
    name: str = "trend_pullback_repair"
    initial_capital: float = 1_000_000.0
    max_positions: int = 10
    strong_5d_threshold: float = 0.03
    signal_day_drop_threshold: float = -0.02
    trend_touch_tolerance: float = 0.03
    confirm_window_days: int = 4
...
class TrendPullbackRepairStrategy(BaseSignalStrategy):
    ...
    def prepare(self, stock_data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        for code, raw_df in stock_data.items():
            ...
            df["touch_ok"] = (
                df["low"].le(df["short_trend"] * (1.0 + self.config.trend_touch_tolerance))
                | df["low"].le(df["long_trend"] * (1.0 + self.config.trend_touch_tolerance))
            )
            df["pullback_candidate"] = (
                ...
                & df["touch_ok"]
                & df["close"].ge(df["long_trend"] * 0.97)
            )

            recent_candidate = pd.Series(False, index=df.index)
            for i in range(1, self.config.confirm_window_days + 1):
    ...
    def generate_signals(self, stock_data: Dict[str, pd.DataFrame]) -> Dict[pd.Timestamp, List[Signal]]:
        ...
        for code, df in stock_data.items():
            for dt in df.index[df["confirmation_bar"].fillna(False)]:
                row = df.loc[dt]
                score = float(row["return_5d"] - abs(row["close"] / row["short_trend"] - 1.0))
    ...
    def build_position(self, signal: Signal, exec_row: pd.Series) -> Dict[str, float]:
        ...
        return {
            "stop_price": signal_low * self.config.stop_loss_buffer,
    ...
    def should_exit(
        ...
        if not partial_taken:
            ...
            if hold_days <= self.config.partial_take_profit_days and gain >= self.config.partial_take_profit_pct:
                ...
                position.metadata["partial_take_price"] = float(row["close"])
...
class TrendPullbackRepairEngine(BacktestEngine):
    def run(self, strategy: BaseSignalStrategy, stock_data: Dict[str, pd.DataFrame], all_dates: List[pd.Timestamp]):
        # Override only the sell behavior for partial_take_profit; otherwise reuse base logic via copied flow.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Formal backtest for trend pullback repair strategy")
    ...
    parser.add_argument(
        "--output-dir",
        default="/Users/lidongyang/Desktop/Qstrategy/results/trend_pullback_repair_backtest_20260311",
    )
    ...
    payload = {
        ...
        "config": {
            ...
            "trend_touch_tolerance": config.trend_touch_tolerance,
            "confirm_window_days": config.confirm_window_days,
