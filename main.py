import time
import random
from utils import B2_brick_filter, b1filter, pinfilter, brick_filter, holdprint, selectprint, stockDataValidator, stoploss, takeprofit
...
if __name__ == '__main__':
    ...
    for file_path in file_paths:
        ...
        # 针对brick+B2融合策略进行校验
        b2_brick_result = B2_brick_filter.check(str(file_path), hold_list)
        if b2_brick_result[0] == 1:
            b2_brick_list.append([
                file_name,
                str(round(b2_brick_result[1], 2)),
                str(round(b2_brick_result[2], 2)),
                str(b2_brick_result[3]),
                str(b2_brick_result[4]),
            ])
