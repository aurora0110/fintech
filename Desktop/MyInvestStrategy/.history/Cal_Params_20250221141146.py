...
def cal_KDJ(data, days, price_type):
    data['high_kdj'] = data[price_type].rolling(days).max()
    data['low_kdj'] = data[price_type].rolling(days).min()
    data['RSV'] = (data[price_type] - data['low_kdj']) / (data['high_kdj'] - data['low_kdj']) * 100
