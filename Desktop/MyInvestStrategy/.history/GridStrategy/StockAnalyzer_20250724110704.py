...
class StockMonitor:
    ...
    def bs_abnormal_monitor(self):
        ...
        price_dict =         analyzer = StockAnalyzer(self.ticker, file_path)
.calculate_price()
        open_price = price_dict['open_price']
        close_price = price_dict['close_price']
        if (close_price < open_price) and (buyvolume_amount > sellvolume_amount):
            print(f"当日绿线📉，但是买入量大于卖出量，可能是有人偷偷在低位收筹码")
        elif(close_price > open_price) and (buyvolume_amount < sellvolume_amount):
            print(f"当日红线📈，但是买入量小于卖出量，可能是有人偷偷在高位卖筹码")
        else:
            print(f"成交量无异常")

        if (close_price < open_price) and (buyprice_amount > sellprice_amount):
            print(f"当日绿线📉，但是买入总额大于卖出总额，可能是有人偷偷在低位收筹码")
        elif(close_price > open_price) and (buyprice_amount < sellprice_amount):
            print(f"当日红线📈，但是买入总额小于卖出总额，可能是有人偷偷在高位卖筹码")
        else:
            print(f"成交总额无异常")

        print(f"卖出总额：{sellprice_amount}，卖出总量：{sellvolume_amount}，买入总额：{buyprice_amount}，买入总量：{buyvolume_amount}")
