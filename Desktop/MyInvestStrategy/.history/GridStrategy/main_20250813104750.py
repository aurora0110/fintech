...
if __name__ == '__main__':
    ...
    for symbol in tqdm(stock_symbol_list):
        ...
        data_price = analyzer.calculate_price()
        data_shakeout = analyzer.calculate_shakeout()
        data_tradeing_sideways = analyzer.tradeing_sideways()
        # 画图
        if figSwitch:
            analyzer.plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, data_shakeout, symbol, windows=[20, 30, 60, 120])
        ...
        # 横盘监控
        if data_tradeing_sideways:
            TRADEING_SIDEWAYS_boolean = True
        ...
        # 横盘且单针信号
        if TRADEING_SIDEWAYS_boolean and ShakeOut_Monitor:
    ...
    print(f"✅买入信号STOCK当日满足四线归零买且J值较低的有{len(stock_four_line_zero_buy_J_list)}个：{stock_four_line_zero_buy_J_list}")
    print(f"✅买入信号STOCK当日满足J值小于{J_threshold}的,且MACD水上💦的有{len(stock_select_list_JM)}个：{stock_select_list_JM}\n⬇️单针下20信号的有{len(stock_select_list_S)}个:{stock_select_list_S}")
    print(f"✅买入信号STOCK当日满足J值小于{J_threshold}，单针下20短期指标小于20且长期指标大于60的有{len(stock_select_list_JS)}个：{stock_select_list_JS}")
    print(f"✅买入信号STOCK当日满足J值小于{J_threshold}，单针下20短期指标小于20且长期指标大于60，最近连续{bbi_days}天的收盘价格大于bbi的有{len(stock_select_list_JSBBI)}个：{stock_select_list_JSBBI}")
    print(f"✅买入信号STOCK当日满足J值快速下降（3天内下降值>=60）的有{len(stock_fast_down_j_list)}个：{stock_fast_down_j_list}，满足连续2天出现洗盘信号的有{len(stock_2days_shakeout_list)}个：{stock_2days_shakeout_list}，满足10天出现3次洗盘信号的有{len(stock_5days_shakeout_list)}个：{stock_5days_shakeout_list}")
    print(f"✅买入信号STOCK当日满足横盘且单针的有{len(stock_select_list_TS)}个：{stock_select_list_TS}，满足横盘且快速勾到负值的有{len(stock_select_list_TJ)}个：{stock_select_list_TJ}")
    ...
    print(f"🥳卖出信号持有且大于9️⃣0️⃣的有{len(stock_select_list_J_sell)}个：{stock_select_list_J_sell}")
    print(f"🥳卖出信号持有且最近2日收盘价跌破BBI线的有{len(stock_below_bbi_list)}个：{stock_below_bbi_list}")