...
def download_daily_trade_volume(symbol, retry_times=3):
    for _ in range(retry_times):
        try:
            ...
            # 查询数据
            stock_data = stock_zh_a_spot_df[stock_zh_a_spot_df["代码"] == full_code]
            print(stock_data)
            if not stock_data.empty:
                return stock_data.iloc[0]  # 返回第一条匹配记录
            
        except Exception as e:
            print(f"第{_+1}次获取{market}{symbol}数据失败，错误: {str(e)}")
...
if __name__ == '__main__':
    ...
    download_daily_trade_volume('000001',3)
