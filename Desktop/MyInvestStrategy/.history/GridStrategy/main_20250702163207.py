...
if __name__ == '__main__':
    ...
    for symbol in symbol_list:
        ...
        print(f"今日：{data.iloc[-1]['日期']}，{symbol}，收盘价为：{data.iloc[-1]['收盘']}，最高价为：{data.iloc[-1]['最高']}，最低价为：{data.iloc[-1]['最低']}，J值为：{data_kdj['J'].iloc[-1]}，单针下20短期指标为：{data_shakeout['短期'][-1]}，单针下20长期指标为：{data_shakeout['长期'][-1]}")
