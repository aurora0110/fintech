...
if __name__ == '__main__':
    ...
    for line, lab in zip(lines, labels):
        line.set_label(lab)
    plt.legend(loc="best")
    plt.title("各宽基指数PE图（近1年） Chart by 格物致知") #PE
    ...
    for line, lab in zip(lines, labels):
        line.set_label(lab)
    plt.legend(loc="best")
    plt.title("各宽基指数图（近1年） Chart by 格物致知") #PE
    #plt.xlabel("Chart by 格物致知")
    plt.ylabel("PE")
    plt.grid(True, alpha=0.25)
    ax2 = plt.gca()
    ...
    lines2 = plt.plot(x_vecs2[0], vecs2[0], x_vecs2[1], vecs2[1], x_vecs2[2], vecs2[2], x_vecs2[3], vecs2[3], x_vecs2[4], vecs2[4],x_vecs2[5], vecs2[5], x_vecs2[6], vecs2[6], x_vecs2[7], vecs2[7], x_vecs2[8], vecs2[8], x_vecs2[9], vecs2[9],x_vecs2[10], vecs2[10], linewidth=1.8)
    # 给每条线设置标签
    labels2 = [f"{x}" for x in industry_list_name]   # 自己改成更有意义的名字
    for line, lab in zip(lines2, labels2):
        line.set_label(lab)
    plt.legend(loc="best")
    plt.title("各行业指数PE图（近1年） Chart by 格物致知") # PE
    #plt.xlabel("Chart by 格物致知")
    plt.ylabel("PE")
    plt.grid(True, alpha=0.25)
    ax3 = plt.gca()
