...
注意：stock_zh_index_spot_em 是“现货快照”，我们用它来找“指数代码与名称”，
...
SPOT_WATCH = {
    "上证系列指数": ["上证综指"],                # 000001
    "深证系列指数": ["深证综合指数", "深证成指", "创业板指"],  # 399106 / 399001 / 399006
    "中证系列指数": ["沪深300","中证500","中证1000","中证全指","上证50"],  # 000300 / 000905 / 000852 / 000985 / 000016
}
...
def get_spot_list(category: str) -> pd.DataFrame:
    ...
    # 常见列：'序号','指数代码','指数名称','涨跌幅','最新价',... 不同版本略有差异
    df.columns = [str(c).strip() for c in df.columns]
    print(df)
    # 只保留我们常用的列
    keep_cols = [c for c in df.columns if c in ["指数代码","指数名称","最新价","涨跌幅","今开","最高","最低","成交量","成交额","更新时间","市场"]]
    return df[keep_cols].copy()

def em_symbol_from_code(code: str) -> str:
    """将 6位指数代码映射为 Eastmoney 的 symbol：000xxx -> sh000xxx；399xxx -> sz399xxx"""
    code = str(code)
...
def get_sw_monthly_valuation(level="一级行业", last_years=5) -> pd.DataFrame:
    ...
    for m in months:
        try:
            ...
            out.append(d[["日期","指数代码","指数名称","市盈率","市净率"]])
        except Exception:
            pass
    if not out:
        return pd.DataFrame()
    big = pd.concat(out, ignore_index=True).drop_duplicates(subset=["日期","指数代码"])
    return big.sort_values(["指数名称","日期"])
...
def build_symbol_map_from_spot(watch: dict) -> dict:
    ...
    for cat, name_keys in watch.items():
        ...
        for key in name_keys:
            hit = spot.loc[spot["指数名称"].astype(str).str.contains(key, regex=False, na=False)]
            if hit.empty:
                print(f"[WARN] {cat} 未找到：{key}")
                continue
            row = hit.iloc[0]
            code = str(row["指数代码"]).zfill(6)
...
def plot_industry_pe(sw_monthly: pd.DataFrame, fn="chart_1_industry_PE.png"):
    ...
    names = (sw_monthly.groupby("指数名称")["日期"].max()
             .sort_values(ascending=False).index.tolist())[:10]
    dfp = sw_monthly[sw_monthly["指数名称"].isin(names)]
    plt.figure(figsize=(12,7), dpi=140)
    for name, g in dfp.groupby("指数名称"):
...
def plot_allA_PB_vs_SZ(fn="chart_3_allA_PB_vs_SZ.png"):
    ...
    for m in months:
        try:
            ...
            d = d.rename(columns={"发布日期":"日期"})
            d["日期"] = pd.to_datetime(d["日期"])
            d = d[d["指数名称"].str.contains("全A")]
            out.append(d[["日期","指数名称","市净率"]].rename(columns={"市净率":"PB"}))
        except Exception:
            pass
    if not out:
