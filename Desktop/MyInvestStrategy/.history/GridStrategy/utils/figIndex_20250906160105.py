...
def distinct_colors(n: int):
    # ≤20 用 tab20 + 重排，>20 用 HSV 等间隔
    import matplotlib.cm as cm
    if n <= 20:
        order = [0,10,2,12,4,14,6,16,8,18, 1,11,3,13,5,15,7,17,9,19][:n]
        cmap = cm.get_cmap('tab20')
        return [cmap(i/19.0) for i in order]
    else:
        hues = np.linspace(0, 1, n, endpoint=False)
        return [mcolors.hsv_to_rgb((h, 0.9, 0.9)) for h in hues]

import numpy as np
import pandas as pd
from matplotlib.ticker import MultipleLocator, AutoMinorLocator, StrMethodFormatter

def set_smart_y_ticks(ax, series_list, target_ticks=7):
    """根据数据动态选择“好看”的刻度步长，并格式化千分位"""
    y = pd.concat([s.dropna() for s in series_list])
    lo, hi = float(y.min()), float(y.max())
    if hi <= lo:
        hi = lo + 1.0
    rng = hi - lo
    # 选一个接近 rng/target_ticks 的“好数字”作为步长
    nice_pool = np.array([1,2,2.5,5,10,20,25,50,100,200,250,500,1000,2000,5000,10000])
    step = nice_pool[np.argmin(np.abs(nice_pool - rng/target_ticks))]

    ax.yaxis.set_major_locator(MultipleLocator(step))
    ax.yaxis.set_minor_locator(AutoMinorLocator(2))
    ax.yaxis.set_major_formatter(StrMethodFormatter('{x:,.0f}'))  # 千分位
    # 留一点上下边距
...
if __name__ == '__main__':
    ...
    for x in index_list:
        ...
        x_col, y_col, x_is_date = fig(file_path,"日期","滚动市盈率")
        vecs.append(y_col)
        X = x_col
    # 5) 画图（不指定颜色，遵循默认配色）
    ...
    # 给每条线设置标签
    labels = [f"{x}" for x in index_list_name]   
    for line, lab in zip(lines, labels):
        line.set_label(lab)
    plt.legend(loc="best")
    plt.title("各宽基指数估值图（近1年）") #PE
    ...
    for x in index_list:
        file_path = index_save_path + x + '.csv'
        x_col, y_col, x_is_date = fig(file_path,"日期","收盘")
        vecs_broad_index.append(y_col)
        X = x_col
    # 5) 画图（不指定颜色，遵循默认配色）
    plt.figure(figsize=(40, 10))
    lines = plt.plot(
        X, vecs_broad_index[0], X, vecs_broad_index[1], X, vecs_broad_index[2], X, vecs_broad_index[3], X, vecs_broad_index[4],
        X, vecs_broad_index[5],X, vecs_broad_index[6],X, vecs_broad_index[7],X, vecs_broad_index[8],
        linewidth=1.8
    )
    ...
    if x_is_date:
        ...
        ax2.xaxis.set_major_formatter(mdates.ConciseDateFormatter(major))
        ax2.xaxis.set_minor_locator(minor)

    # 纵轴刻度更细（按你的PE范围调整步长）
    set_smart_y_ticks(ax2, vecs_broad_index)  
    ax2.yaxis.set_minor_locator(AutoMinorLocator(2))    # 每个主刻度再分 2 份
    ...
    if x_is_date:
        ...
        ax3.xaxis.set_major_locator(major2)
        ax3.xaxis.set_major_formatter(mdates.ConciseDateFormatter(major2))
        ax3.xaxis.set_minor_locator(minor2)
    ...
    # 刻度更小
    ax3.tick_params(axis="both", which="major", labelsize=9, length=4)
    ax3.tick_params(axis="both", which="minor", labelsize=0, length=2)

    plt.tight_layout()

    # ----------------------------------------4、行业指数图----------------------------------------
    industry_index_vecs2 = []
    for x in industry_list:
        file_path = index_save_path + x + '.csv'
        x_col, y_col, x_is_date = fig(file_path,"日期","收盘")
        industry_index_vecs2.append(y_col)
        x_vecs2.append(x_col)
    print(len(vecs2))
    # 5) 画图（不指定颜色，遵循默认配色
    plt.figure(figsize=(40, 10))

    lines2 = plt.plot(x_vecs2[0], industry_index_vecs2[0], x_vecs2[1], industry_index_vecs2[1], x_vecs2[2], industry_index_vecs2[2], x_vecs2[3], industry_index_vecs2[3], x_vecs2[4], industry_index_vecs2[4],x_vecs2[5], industry_index_vecs2[5], x_vecs2[6], industry_index_vecs2[6], x_vecs2[7], industry_index_vecs2[7], x_vecs2[8], industry_index_vecs2[8], x_vecs2[9], industry_index_vecs2[9],x_vecs2[10], industry_index_vecs2[10], x_vecs2[11], industry_index_vecs2[11],x_vecs2[12], industry_index_vecs2[12],x_vecs2[13], industry_index_vecs2[13],x_vecs2[14], industry_index_vecs2[14],x_vecs2[15], industry_index_vecs2[15],x_vecs2[16], industry_index_vecs2[16],linewidth=2)
    # 给每条线设置标签
    labels2 = [f"{x}" for x in industry_list_name]   # 自己改成更有意义的名字
    for line, lab in zip(lines2, labels2):
        line.set_label(lab)
    plt.legend(loc="best")
    plt.title("各行业指数图（近1年）") # PE
    plt.xlabel("Chart by 格物致知")
    plt.ylabel("INDEX")
    plt.grid(True, alpha=0.25)
    ax4 = plt.gca()
    ...
    if x_is_date:
        major2 = mdates.MonthLocator(interval=1)
        minor2 = mdates.WeekdayLocator(byweekday=mdates.MO)
        ax4.xaxis.set_major_locator(major2)
        ax4.xaxis.set_major_formatter(mdates.ConciseDateFormatter(major2))
        ax4.xaxis.set_minor_locator(minor2)

    # 纵轴更细
    set_smart_y_ticks(ax4, industry_index_vecs2)  
    ax4.yaxis.set_minor_locator(AutoMinorLocator(2))

    # 刻度更小
    ax4.tick_params(axis="both", which="major", labelsize=9, length=4)
    ax4.tick_params(axis="both", which="minor", labelsize=0, length=2)
