...
def get_stock_market_value(stock_code):
    ...
    try:
        ...
        stock_data = stock_zh_a_spot_df[stock_zh_a_spot_df["代码"] == f"sh{stock_code}" if stock_code.startswith("6") else f"sz{stock_code}"]
        if not stock_data.empty:
            return stock_data.iloc[0]["总市值"]  # 单位：亿元人民币
    except Exception as e:
        print(f"获取数据失败: {e}")
    return None
