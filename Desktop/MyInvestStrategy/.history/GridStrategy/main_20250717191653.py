...
if __name__ == '__main__':
    ...
    if downloadNewDataSwitch:
        ...
        stock_data = getData.batch_download_stock_data(all_stock_symbol_list, max_workers=30, start_date=stock_start_date, end_date=end_date, year_interval=1)
    ...
    for symbol in stock_symbol_list:
        ...
        with open(backtest_log_path_new, 'a') as f:
            f.write(f'*************当前回测策略为：可投入金额为{amount}元，最小操作间隔为{ineterval_days}天，计划操作手数为{total_shares}手*************')    
        print(f"⏰今日：{data.iloc[-1]['日期']}，{symbol}，收盘价为：{data.iloc[-1]['收盘']}，最高价为：{data.iloc[-1]['最高']}，最低价为：{data.iloc[-1]['最低']}，J值为：{round(data_kdj['J'].iloc[-1],3)}，MACD值为：{round(data_macd['DIF'].iloc[-1],3)}，单针下20短期指标为：{round(data_shakeout['短期'].iloc[-1],3)}，单针下20长期指标为：{round(data_shakeout['长期'].iloc[-1],3)}")
        print(f"💹技术指标：J值小于-5：{'true✅' if J_boolean else 'false❌'}，MACD指标：DIF水上：{'true✅' if MACD_boolean else 'false❌'}，单针下20短期指标小于20且单针下20长期指标大于60：{'true✅' if SHAKEOUT_boolean else 'false❌'}，最近连续{bbi_days}天的收盘价格大于bbi：{'true✅' if BBI_boolean else 'false❌'}，3天内J快速下降：{'true✅' if fast_down_j_label else 'false❌'}")
        print("🐤" * 95)
    ...
    print(f"✅STOCK当日满足J值小于-5️⃣的有{len(stock_select_list_J)}个：{stock_select_list_J}，❗️持有且大于9️⃣0️⃣的有{len(stock_select_list_J_sell)}个：{stock_select_list_J_sell}，⬇️单针下20信号的有{len(stock_select_list_S)}个:{stock_select_list_S}，J值快速下降的有{len(stock_fast_down_j_list)}个：{stock_fast_down_j_list}")
