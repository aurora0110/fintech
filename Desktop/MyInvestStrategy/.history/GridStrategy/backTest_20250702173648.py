...
def backTest(file, amount, windows, total_shares, each_buy_shares, start_date, end_date):
    ...
    for i in range(len(df)):
        ...
        # 卖出逻辑 ---------------------------------》这里可以添加上其他的策略作为限制条件，看回测效果如何？比如20250702是否加一个当盈利达到一定比例再卖出？
    ...
    while i < len(sell_list) and j < len(buy_list):
        ...
        if sell_date > buy_date:
            if sell_price > buy_price:
                ...
                delta_days = datetime.strptime(sell_date, '%Y-%m-%d') - datetime.strptime(buy_date, '%Y-%m-%d')
                print(f'sell_price:{sell_price}, sell_date:{sell_date}, buy_price:{buy_price}, buy_date:{buy_date}, profit:{profit}, holddays:{delta_days.days}')
                buy_list.pop(j)
                sell_list.pop(i)
