...
def plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, ticker, windows):
    ...
    x_axis = pd.to_datetime(data_ma['date'])  # 转为 datetime 格式

    # 创建 5 行 1 列的共享 x 轴画布
    fig = sp.make_subplots(rows=5, cols=1, shared_xaxes=True, vertical_spacing=0.02,
                        subplot_titles=[
                            f'MA {windows[0]} {windows[1]} {windows[2]}',
                            'BBI',
                            'Avg & Close Price',
                            'MACD',
    ...
    # 图1 MA
    fig.add_trace(go.Scatter(x=x_axis, y=data_ma[f'MA_{windows[0]}'], name=f'MA_{windows[0]}', line=dict(color='red')), row=1, col=1)
    fig.add_trace(go.Scatter(x=x_axis, y=data_ma[f'MA_{windows[1]}'], name=f'MA_{windows[1]}', line=dict(color='blue')), row=1, col=1)
    fig.add_trace(go.Scatter(x=x_axis, y=data_ma[f'MA_{windows[2]}'], name=f'MA_{windows[2]}', line=dict(color='green')), row=1, col=1)

    # 图2 BBI
    fig.add_trace(go.Scatter(x=x_axis, y=data_bbi['bbi'], name='BBI', line=dict(color='orange')), row=2, col=1)

    # 图3 Avg & Close
    fig.add_trace(go.Scatter(x=x_axis, y=data_price['avg_price'], name='avg_price', line=dict(color='red')), row=3, col=1)
    fig.add_trace(go.Scatter(x=x_axis, y=data_price['close_price'], name='close_price', line=dict(color='blue')), row=3, col=1)

    # 图4 MACD
    fig.add_trace(go.Scatter(x=x_axis, y=data_macd['DIF'], name='DIF', line=dict(color='red')), row=4, col=1)
