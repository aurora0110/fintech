...
def download_stock_data(symbol, type, start_date, end_date, adjust):
    ...
    try:
        if type == "stock":
            #stock_data = ak.stock_zh_a_hist(symbol=symbol, start_date=start_date, end_date=end_date, adjust=adjust) # symbol=symbol, period="daily", start_date=start_date, end_date=end_date, adjust="qfq"
...
def download_etfHistory_data(type, symbol):
    """
    获取单只基金历史数据
    ...
    try:
        if type == "etf":
            symbol = symbol[2:]
            etf_data = ak.fund_etf_hist_em(symbol=symbol, period="daily", start_date="20200101", end_date="20250612", adjust="qfq")
            print(etf_data)
            return etf_data
        elif type == "lof":
            symbol = symbol[2:]
            lof_data = ak.fund_lof_hist_em(symbol=symbol, period="daily", start_date="20200101", end_date="20250612", adjust="qfq")
            return lof_data
        else:
            print("No This Type")
    except Exception as e:
...
def save_2_csv(data, symbol):
    ...
    print(f"正在保存{symbol}{timestamp}数据到csv文件...")
    data.to_csv(f"{symbol}{timestamp}.csv")
...
if __name__ == '__main__':
    ...
    '''
    download_stock_data("sh000001", "stock", "2023-01-01 09:32:00", "2023-01-31 09:32:00","qfq")
    download_etfHistory_data('etf', 'sz159938')
