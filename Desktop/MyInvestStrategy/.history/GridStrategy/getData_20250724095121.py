...
def batch_download_daily_trade_volume(symbol, trade_date, retry):
    '''
    使用腾讯接口，批量获取交易量信息和买卖笔数
    '''
    for _ in range(retry):
        try:
            # 自动识别市场前缀
            market = 'sh' if symbol.startwith('6') else 'sz'
            df = ak.stock_zh_a_tick_tx_js(symbol=f"{market}{symbol}")
            # 清洗数据
            df = df[['成交时间','成交价格','成交金额','成交量','性质']]

        except Exception as e:
            print(f"第{_+1}次获取{symbol}数据失败，重试中...")
            time.sleep(1)
            
    raise Exception(f"获取{symbol}数据失败，重试次数已用完")
