...
if __name__ == '__main__':
    ...
    # 补全ETF代码，读取全市场股票代码和对应名字
    new_etf_symbol_list = []
    for symbol in etf_symbol_list:
        pd = getData.read_from_csv("/Users/lidongyang/Desktop/MyInvestStrategy/GridStrategy/data/全市场etf目录20250612.csv")
        try:
            code = pd.loc[pd['代码'].str[-6:] == symbol, '代码'].values[0]
            name = pd.loc[pd['代码'].str[-6:] == symbol, '名称'].values[0]
            new_etf_symbol_list.append(code)
            print(f"{symbol}对应的完整代码为{code},名称为{name}")
        except IndexError:
            print(f"警告：在ETF目录中找不到代码 {symbol}")
            code = symbol  # 如果找不到，使用代码作为名称
    ...
    if downloadNewDataSwitch:
        data_new = getData.batch_download_etf_data(new_etf_symbol_list, "all", etf_start_date, end_date, 5)
    ...
    holding_codes = []
    # 计算ETF
    for symbol in tqdm(new_etf_symbol_list):
        J_boolean = False
        ...
        backtest_log_path_new = backtest_log_path + name + ".txt"
        ...
        analyzer = StockAnalyzer(name, file_path)
        ...
        # 画图
        if figSwitch:
            analyzer.plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, data_shakeout, name, windows=[20, 30, 60, 120])
        ...
        #data_back = bt.backTest(data_input, amount, ineterval_days, total_shares, each_buy_shares, etf_start_date, end_date, backtest_log_path_new)
         ...
         #   well_list.append([name, f"{round(data_back['avg_profit'] * 100, 3)}%"])
        #else:
         #   ordinary_list.append([name, f"{round(data_back['avg_profit'] * 100, 3)}%"])

        if data_kdj['J'].iloc[-1] <= J_threshold:
            J_boolean = True
        
        if data_kdj['J'].iloc[-1] >= 90 and name in holding_stock_codes:
            select_list_J_sell.append(name)
        ...
        if J_boolean:
            select_list_J.append(name)
        
        if J_boolean and MACD_boolean:
            select_list_JM.append(name)

        if J_boolean and SHAKEOUT_boolean and BBI_boolean:
            select_list_JSBBI.append(name)
    
        if J_boolean and SHAKEOUT_boolean:
            select_list_JS.append(name)
        
        with open(backtest_log_path_new, 'a') as f:
            f.write(f'*************当前回测策略为：可投入金额为{amount}元，最小操作间隔为{ineterval_days}天，计划操作手数为{total_shares}手*************')    
        print(f"⏰今日：{data.iloc[-1]['日期']}，{name}，收盘价为：{data.iloc[-1]['收盘']}，最高价为：{data.iloc[-1]['最高']}，最低价为：{data.iloc[-1]['最低']}，J值为：{round(data_kdj['J'].iloc[-1],3)}，MACD值为：{round(data_macd['DIF'].iloc[-1],3)}，单针下20短期指标为：{round(data_shakeout['短期'].iloc[-1],3)}，单针下20长期指标为：{round(data_shakeout['长期'].iloc[-1],3)}")
    ...
    stock_holding_codes = []
    stock_four_line_zero_buy_J_list = []
    stock_select_list_TS = []
    stock_select_list_TJ = []
    # 计算stock
    for symbol in tqdm(stock_symbol_list):
        ...
        FOUR_LINE_ZERO_BOUGHT_J_signal = False
        TRADEING_SIDEWAYS_boolean = False
        file_path = config.file_path 
        ...
        # 四线归零跟勾值结合
        if data_shakeout['四线归零买'].iloc[-1] == 1 and data_kdj['J'].iloc[-1] <= J_threshold:
            FOUR_LINE_ZERO_BOUGHT_J_signal = True
        ...
        if FOUR_LINE_ZERO_BOUGHT_J_signal:
            stock_four_line_zero_buy_J_list.append([symbol, pd_dict[symbol]])
        ...
        # 横盘监控Label
        TRADING_SIDEWAYS_Monitor = StockMonitor(symbol, file_path, file_volume_path).tradeing_sideways()
        # 放卖飞策略打分
        anti_pre_score = StockMonitor(symbol, file_path, file_volume_path).anti_premature_exit_scoreing_strategy()
        ...
        # 横盘且单针信号
        if TRADEING_SIDEWAYS_boolean and SHAKEOUT_boolean:
            stock_select_list_TS.append([symbol, pd_dict[symbol]])
        # 横盘且快速勾到负值
        if TRADEING_SIDEWAYS_boolean and Jmonitor:
            stock_select_list_TJ.append([symbol, pd_dict[symbol]])
        with open(backtest_log_path_new, 'a') as f:
            f.write(f'*************当前回测策略为：可投入金额为{amount}元，最小操作间隔为{ineterval_days}天，计划操作手数为{total_shares}手************\n')    
            ...
            f.write(f"💹技术指标：J值小于{J_threshold}：{'true✅' if J_boolean else 'false❌'}，四线归零买且J值较低:{'true✅' if FOUR_LINE_ZERO_BOUGHT_J_signal else 'false❌'}，MACD指标：DIF水上：{'true✅' if MACD_boolean else 'false❌'}，单针下20短期指标小于20且长期指标大于60：{'true✅' if SHAKEOUT_boolean else 'false❌'}，最近连续{bbi_days}天的收盘价格大于bbi：{'true✅' if BBI_boolean else 'false❌'}，J值快速下降监控：{'true✅' if Jmonitor else 'false❌'}，连续洗盘信号：{'true✅' if ShakeOut_Monitor else 'false❌'}，最近5天出现3次洗盘信号：{'true✅' if Frequency_Monitor else 'false❌'}，横盘信号：{'true✅' if TRADEING_SIDEWAYS_boolean else 'false❌'}\n")
    ...
    print(f"✅买入信号：J值小于{J_threshold}的+四线归0买，{len(stock_four_line_zero_buy_J_list)}个：{stock_four_line_zero_buy_J_list}")
    print(f"✅买入信号：J值小于{J_threshold}的+MACD水上，{len(stock_select_list_JM)}个：{stock_select_list_JM}\n⬇️单针下20信号的，{len(stock_select_list_S)}个:{stock_select_list_S}")
    print(f"✅买入信号：J值小于{J_threshold}的+单针下20短期指标小于20且长期指标大于60的有{len(stock_select_list_JS)}个：{stock_select_list_JS}")
    print(f"✅买入信号：J值小于{J_threshold}，单针下20短期指标小于20且长期指标大于60，最近连续{bbi_days}天的收盘价格大于bbi的有{len(stock_select_list_JSBBI)}个：{stock_select_list_JSBBI}")
    print(f"✅买入信号：J值快速下降（3天内下降值>=60）的有{len(stock_fast_down_j_list)}个：{stock_fast_down_j_list}，满足连续2天出现洗盘信号的有{len(stock_2days_shakeout_list)}个：{stock_2days_shakeout_list}，满足10天出现3次洗盘信号的有{len(stock_5days_shakeout_list)}个：{stock_5days_shakeout_list}")
    print(f"✅买入信号：横盘+单针的有{len(stock_select_list_TS)}个：{stock_select_list_TS}，横盘+J快速下降的有{len(stock_select_list_TJ)}个：{stock_select_list_TJ}")
    print(f"❗️异常信号STOCK出现异常成交量和成交额的有{len(stock_bs_vol_price_list)}个：{stock_bs_vol_price_list}")
    ...
    print(f"🥳卖出信号：持有且最近2日收盘价跌破BBI线的有{len(stock_below_bbi_list)}个：{stock_below_bbi_list}")