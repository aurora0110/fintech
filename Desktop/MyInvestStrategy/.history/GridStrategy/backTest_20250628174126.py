...
def backTest(file, amount, windows, total_shares):
    '''
    回测
    :param file: 数据文件路径
    :param amount: 初始资金
    ...
    while i < len(sell_list) and j < len(buy_list):
        ...
        if sell_date > buy_date:
            if sell_price > buy_price:
                ...
                print(f'sell_price:{sell_price}, buy_price:{buy_price}, profit:{profit}')
                buy_list.pop(j)
                sell_list.pop(i)
        ...
        if j >= len(buy_list):
            i += 1
            j = 0
    avg_profit = sum(earning_results) / int(end_year - start_year + 1) if earning_results else 0
    print(f'平均年化收益: {avg_profit}')
    return result
...
results = backTest(sample_data, 50000, 30, 20)