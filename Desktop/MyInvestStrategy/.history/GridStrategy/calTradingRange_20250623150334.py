...
def cal_volatility(data, days):
    ...
    std_roll_values = data[['最高', '最低', '收盘']].rolling(days).std().mean()
    std_values = data[['最高', '最低', '收盘']].std().mean()
    print(f'全部数据，{days}天平均滚动价格标准差为：{std_roll_values}，全部数据平均价格标准差为：{std_values}')
    return data

def cal_ATR(data, days):
    '''
    # 计算ATR 平均真实波幅，计算窗口内价格波动的幅度，用于判断价格波动的大小，指标上升，波动程度增加，反之减少
    :param data: dataframe格式的数据
    :param days: 计算窗口
    :return: 
    '''
