...
class StockMonitor:
    ...
    def check_signal_frequency(data_shakeout, lookback_period=5, min_signal_count=3):
        signal_count = 0
        for period in range(-1, -lookback_period-1, -1):  # 检查最近5个周期
            if any([
                ...
                signal_count += 1
                if signal_count >= min_signal_count:  # 达到最小信号数就提前返回
                    return True
        return signal_count >= min_signal_count
