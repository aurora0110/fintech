...
class StockMonitor:
    ...
    def heavy_cannos_monitor(self):
        ...
        df = getData.read_from_csv(self.file_path)
        period_days = 7
        period_df = df.iloc[period_days - 6 : period_days + 1].copy()
        ...
        for i in range(len(period_df) - 1):  # 最后一行是今天，不比较
            ...
            volume_similar = abs(day['volume'] - today_volume) / today_volume <= 0.10
            
            if high_similar and low_similar and volume_similar:
                # 找出这两天中较低的成交量
                vol_min = min(day['volume'], today_volume)
                
                # 这两天之间的所有天的成交量必须都低于这两天的最小成交量
                intermediate = period_df.drop([period_df.index[i], period_df.index[-1]])
                if all(intermediate['volume'] < vol_min):
                    return True  # 满足条件
