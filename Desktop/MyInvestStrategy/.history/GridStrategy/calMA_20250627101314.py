...
# 4.多张图汇集在一张画板上
def plot_all(data_ma, data_bbi, data_price, ticker, windows):
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(18, 6))
    fig.suptitle(f'{ticker} ma bbi points', fontsize = 16)

    plt.figure(figsize=(18, 6)) # 创建画板
    fig.suptitle(f'{ticker} ma bbi points', fontsize = 16)
    ...
    for key, value in data_ma.items():
        if i < len(colors):
            axes[0].plot(x_axis, value, colors[i])
            i += 1
        else:
            pass
    axes[0].plot(x_axis, data_ma[f'MA_{windows[0]}'], 'r-')
    axes[0].plot(x_axis, data_ma[f'MA_{windows[1]}'], 'b-')
    axes[0].plot(x_axis, data_ma[f'MA_{windows[2]}'], 'g-')
    axes[0].set_title(f'MA {windows[0]} {windows[1]} {windows[2]}')
    axes[0].set_xlabel('date')
    axes[0].set_ylabel('price')
    axes[0].grid(True, linestyle='--', linewidth=0.2, alpha=1)  # 增加网格线
    axes[0].legend()

    # 第二张图 bbi
    axes[1] = plt.subplot2grid((2,2),(0,1))
    axes[1].plot(x_axis, data_bbi['bbi'], 'r-')
    axes[1].set_title('bbi')
    axes[1].set_xlabel('date')
    axes[1].set_ylabel('price')
    axes[1].grid(True, linestyle='--', linewidth=0.2, alpha=1)  # 增加网格线
    axes[1].legend()

    # 第二张图 price
    axes[2] = plt.subplot2grid((2,2),(1,1),colspan=2)
    axes[2].plot(x_axis, data_price['avg_price'], 'r-')
    axes[2].plot(x_axis, data_price['close_price'], 'b-')
    axes[2].set_title('bbi')
    axes[2].set_xlabel('date')
    axes[2].set_ylabel('price')
    axes[2].grid(True, linestyle='--', linewidth=0.2, alpha=1)  # 增加网格线
    axes[2].legend()
