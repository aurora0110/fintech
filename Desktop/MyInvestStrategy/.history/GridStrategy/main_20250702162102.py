...
if __name__ == '__main__':
    ...
    for symbol in symbol_list:
        ...
        print(f"今日：{data.iloc[-1]["日期"]}，{symbol}，收盘价为：{data.iloc[-1]["收盘"]}，最高价为：{data.iloc[-1]["最高"]}，最低价为：{data.iloc[-1]["最低"]}，J值为：{data_kdj['J'][-1]}，单针下20短期指标为：{data_shakeout['短期'][-1]}，单针下20长期指标为：{data_shakeout['长期'][-1]}")
        print(f"满足条件：J值小于-5：{J_boolean}，单针下20短期指标小于20且单针下20长期指标大于60：{SHAKEOUT_boolean}，最近连续10天的收盘价格大于bbi：{BBI_boolean}")
        print("***********" * 10)

    print(f"回测策略年化收益大于5%：{well_list}", )
    print(f"回测策略年化收益小于5%：{ordinary_list}", )
