...
if __name__ == '__main__':
    ...
    for symbol in tqdm(etf_symbol_list):
        ...
        if J_boolean:
            select_list_J.append([symbol, name])
        
        if J_boolean and MACD_boolean:
            select_list_JM.append([symbol, name])

        if J_boolean and SHAKEOUT_boolean and BBI_boolean:
            select_list_JSBBI.append([symbol, name])
    
        if J_boolean and SHAKEOUT_boolean:
            select_list_JS.append([symbol, name])
    ...
    with open(everyday_abnormal_volume_path, 'a') as f:
        ...
        f.write(f"✅买入信号STOCK当日满足四线归零买且J值较低的有{len(stock_four_line_zero_buy_J_list)}个：{stock_four_line_zero_buy_J_list}\n")    
