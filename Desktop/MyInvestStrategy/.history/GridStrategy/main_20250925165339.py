...
if __name__ == '__main__':
    ...
    for symbol in etf_symbol_list:
        pd_all_etf = getData.read_from_csv("/Users/lidongyang/Desktop/MyInvestStrategy/GridStrategy/data/全市场etf目录20250612.csv")
        try:
            code = pd_all_etf.loc[pd_all_etf['代码'].str[-6:] == symbol, '代码'].values[0]
            name = pd_all_etf.loc[pd_all_etf['代码'].str[-6:] == symbol, '名称'].values[0]
    ...
    for symbol in tqdm(stock_symbol_list):
        ...
        #data_back = bt.backTest(data_input, amount, ineterval_days, total_shares, each_buy_shares, stock_start_date, end_date, backtest_log_path_new)
         ...
         # 对齐日期与 J（calculate_kdj 返回的 Series 与 stock_data 同索引）
        df_sorted = analyzer.stock_data.sort_values('日期').reset_index(drop=True)
        j_series = data_kdj['J'].reset_index(drop=True)
        daily_j = pd.DataFrame({
            '日期': df_sorted['日期'],
            'J': j_series
        })
        # —— 今日 J 在“历史（含今日）”的百分位（平均法，= pandas 的 rank 平均并归一化）——
        pct_including_today = float(daily_j['J'].rank(pct=True, method='average').iloc[-1] * 100)
        print(f"今日J在历史（含今日）的百分位：{pct_including_today:.2f}%")
        ...
        # 读取全市场股票代码和对应名字
        pd_file = getData.read_from_csv("/Users/lidongyang/Desktop/MyInvestStrategy/GridStrategy/data/全市场etf目录20250722.csv")
        # 读取的代码左侧缺0，补0
        pd_file["code"] = pd_file["code"].fillna(0).astype(int).astype(str).str.zfill(6)
        pd_dict = pd_file.set_index("code")["name"].to_dict()
