...
class StockMonitor:
    ...
    def fastdown_J(self):
        ...
        for period in [-1, -2, -3] )
        print(f"{self.ticker}J值是否快速下降：{label}，最近3️⃣天的J值：{round(data_kdj['J'].iloc[-3],1)}，{round(data_kdj['J'].iloc[-2],1)}，{round(data_kdj['J'].iloc[-1],1)}")
    ...
    def continuous_shakeout(self):
        ...
        label = False
        print(data_shakeout.iloc[-1][-4:], data_shakeout.iloc[-2][-4:], data_shakeout.iloc[-3][-4:])
        label = all(
        any([
