...
if __name__ == '__main__':
    ...
    for symbol in symbol_list:
        ...
        # 计算MA bbi均线并画图
        data_ma = cm.calculate_moving_averages(data, start_date, end_date, windows)
        # cm.plot_moving_averages(data_ma, symbol, colors, 'MA')
        data_bbi = cm.calculate_bbi(data, start_date, end_date)
        # cm.plot_moving_averages(data_bbi, symbol, [colors[0]], 'BBI')
        data_price = cm.calculate_price(data, start_date, end_date)
        # 计算MACD
        data_macd = calMACD.cal_macd(data, start_date, end_date, 12, 26, '收盘')
        # 计算kdj
        ...
        data_back = bt.backTest(data_input, 200000, 1, 10, start_date, end_date)
        ...
        bbi_days = 10
        bbi_last = data_bbi.tail(bbi_days).reset_index(drop=True)
        ...
        print(f"今日：{data.iloc[-1]['日期']}，{symbol}，收盘价为：{data.iloc[-1]['收盘']}，最高价为：{data.iloc[-1]['最高']}，最低价为：{data.iloc[-1]['最低']}，J值为：{data_kdj['J'].iloc[-1]}，单针下20短期指标为：{data_shakeout['短期'].iloc[-1]}，单针下20长期指标为：{data_shakeout['长期'].iloc[-1]}")
    ...
    print(f"回测策略年化收益小于5%{len(ordinary_list)}个：{ordinary_list}")
