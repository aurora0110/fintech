...
def plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, ticker, windows):
    ...
    # 第二行，整行 price
    fig.add_trace(go.Scatter(x=x_axis, y=data_price['avg_price'], name='avg_price', line=dict(color='white')), row=2, col=1)
    fig.add_trace(go.Scatter(x=x_axis, y=data_price['close_price'], name='close_price', line=dict(color='yellow')), row=2, col=1)
