...
class StockMonitor:
    ...
    def continuous_shakeout(self):
        ...
        label = all(
        any([
            data_shakeout[period].get("四线归零买", False) == True,
            data_shakeout[period].get("白线下20买", False) == True,
            data_shakeout[period].get("白穿红线买", False) == True,
            data_shakeout[period].get("白穿黄线买", False) == True
        ])
        for period in ["-1", "-2", "-3"]
    )
        return label
