...
def plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, ticker, windows):
    ...
    ax4.plot(x_axis, data_macd['DIF'], 'r-') # 快白线
    ax4.plot(x_axis, data_macd['DEA'], 'y-') # 慢黄线
    ax4.set_title('macd')
    #ax3.set_xlabel('date')
    ax4.set_ylabel('')
    ...
    # 第五张图 
    # 1. 转换字符串日期为 datetime 对象
    x_axis_dt = pd.to_datetime(x_axis).to_pydatetime()

    # 2. 获取需要标注的点
    mask = data_kdj['J'] <= -5
    highlight_x = [x for x, m in zip(x_axis_dt, mask) if m]
    highlight_y = data_kdj['J'][mask]

    # 3. 开始绘图
    fig, ax5 = plt.subplots(figsize=(12, 5))

    ax5.xaxis_date()  # ✅ 明确声明：x 轴是日期，不是分类

    # 4. 画主曲线
    ax5.plot(x_axis_dt, data_kdj['J'], label='KDJ-J')

    # 5. 画红点
    ax5.scatter(highlight_x, highlight_y, color='red', zorder=5)

    # 6. 标注箭头
    for date, val in zip(highlight_x, highlight_y):
        ax5.annotate(
            f'{val:.2f}',
            xy=(date, val),
            xytext=(date, val + 0.5),
    ...
    # 7. 美化图表
    ax5.set_title('KDJ-J values <= -5 Highlighted')
    ax5.set_xlabel('Date')
    ax5.set_ylabel('J Value')
    ax5.legend()
    fig.autofmt_xdate()  # 自动旋转日期显示
    plt.tight_layout()
    plt.show()

    '''
    ax5 = plt.subplot2grid((4,2),(3,0),colspan=2)
    ax5.plot(x_axis, data_kdj['J'], 'r-') 
    ax5.set_title('J')
    #ax3.set_xlabel('date')
    ax5.set_ylabel('')
    ax5.grid(True, linestyle='--', linewidth=0.2, alpha=1)  # 增加网格线
    

    # 1. 转换日期字符串为 datetime 对象
    x_axis_dt = pd.to_datetime(x_axis).to_pydatetime()  # 转为 datetime 类型以防 matplotlib 报错
