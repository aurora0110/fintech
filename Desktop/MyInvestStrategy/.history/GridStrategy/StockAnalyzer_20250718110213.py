...
class StockMonitor:
    ...
    def fastdown_J(self):
        ...
        print(f"{self.ticker}的最近3️⃣天的J值：{data_kdj['J'].iloc[-3]}，{data_kdj['J'].iloc[-2]}，{data_kdj['J'].iloc[-1]}")

        if data_kdj['J'].iloc[-1] < 10 and data_kdj['J'].iloc[-3] > 70:
            label = True
        return label
    ...
    def continuous_shakeout(self):
        ...
        print(type(data_shakeout),data_shakeout)
        print(data_shakeout.iloc[-1]["四线归零买"])
        label = all(
        any([
            data_shakeout.iloc[period].get("四线归零买", False) == True,
            data_shakeout.iloc[period].get("白线下20买", False) == True,
            data_shakeout.iloc[period].get("白穿红线买", False) == True,
            data_shakeout.iloc[period].get("白穿黄线买", False) == True
        ])
        for period in [-1, -2, -3] )
...
if __name__ == "__main__":
    ...
    label = StockMonitor(ticker, file_path).continuous_shakeout()
