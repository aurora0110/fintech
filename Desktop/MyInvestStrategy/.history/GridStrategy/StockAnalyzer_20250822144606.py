...
class StockAnalyzer:
    ...
    def calculate_vcp(self, col_date: str = "日期", col_open: str = "开盘",col_high: str = "最高",col_low: str = "最低",col_close: str = "收盘", col_volume: str = "vol成交量ume",# —— ZigZag & VCP 参数 —— 
                    zigzag_pct: float = 0.06,         # ZigZag 反转阈值（6%）——建议 4%~10% 之间调
                    min_contractions: int = 2,        # 至少需要几段 "peak->trough" 收缩
                    max_contractions: int = 4,        # 最多考虑几段收缩用于评分
                    min_drop: float = 0.06,           # 每段最小回撤比例（≥6%）
                    tighten_ratio_max: float = 0.85,  # 连续收敛阈值：C2 <= C1 * 0.85
                    dryup_days: int = 5,              # 枢轴前统计量能干涸的天数
                    vol_ma_len: int = 50,             # 成交量均线长度
                    dryup_thresh: float = 0.60,       # 干涸阈值：近 dryup_days 均量 / MA(vol_ma_len) < 0.6
                    bb_len: int = 20,                 # 布林带窗口
                    bw_low_pct: float = 0.35,         # 带宽历史低分位（35%）
                    atr_len: int = 14,                # ATR%窗口
                    atr_low_pct: float = 0.35,        # ATR%历史低分位（35%）
                    near_pivot_tol: float = 0.03,     # 现价距枢轴下方 ≤3% 视为接近枢轴
                    breakout_vol_mult: float = 1.5,   # 若已突破：当日量 > vol_ma_len 均量 * 1.5
                    window: int = 200,                # 仅在最近 window 根K内评估（防止历史过长稀释分位）
    ):
        ...
        # 数值化
        df = to_numeric_df(df, [c for c in ["开盘", "最高", "最低", "收盘", "成交量"] if c in df.columns])
        ...
        df["vol_ma"] = df["成交量"].rolling(vol_ma_len, min_periods=1).mean()
        ...
        pivot_price = float(df.loc[seg[-1]["peak_idx"], "最高"])
        last_close = float(df["最低"].iloc[-1])
        ...
        if breakout:
            breakout_vol_ok = (df["成交量"].iloc[-1] > breakout_vol_mult * df["vol_ma"].iloc[-1])
    ...
    def evaluate_ma_structure(
        ...
        """
        优秀均线结构（多头排列 & 上行 & 不过度乖离）：
        - 现价在所有MA之上
        - 短期MA > 中期MA > 长期MA（严格有序）
        - 每条MA最近 slope_lookback 根的方向为上（MA[-1] > MA[-1 - slope_lookback]）
        - （可选）不过度扩张：|Close/MA_short - 1| <= max_extension
        ...
        ma = {w: s.rolling(w, min_periods=w).mean() for w in windows}
        last = s.iloc[-1]
        ...
        # 打分
        score = 0.0
        score += 25 if above_all else 0
        score += 40 if ordered else 0
        score += 25 if slopes_ok else 0
