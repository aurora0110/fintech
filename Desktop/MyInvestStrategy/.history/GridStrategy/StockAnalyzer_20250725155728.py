...
class StockMonitor:
    ...
    def bs_abnormal_monitor(self):
        ...
        * 开盘收盘价格是从000001.csv（历史价格）文件中获取的，开盘收盘总价和总量是从000001_volume.csv（只有每天最新的价格）文件中获取的，如果想看历史数据可以去通达信导出
        ...
        # 获取总市值和总股本
        market_cap, share_cap = getData.download_daily_download_total_captrade_volume(self.ticker)
        ...
        print(f"当前异常交易占总股本比重:{abs(buyprice_amount-sellprice_amount) / int(share_cap)}，占总市值比重为:{abs(buyprice_amount-sellprice_amount) / int(market_cap)}")
