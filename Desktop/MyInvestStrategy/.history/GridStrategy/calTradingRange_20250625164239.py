...
def cal_range(data, days):
    ...
    value = round(data['Range'].tolist()[-1], 3)

    print(f'全部数据，{days}天最高价格与最低价格波动差为：{value}')
    return data
...
def cal_volatility(data, days):
    ...
    std_roll_values = round(data[['最高', '最低', '收盘']].rolling(days).std().mean(), 3)
    std_values = round(data[['最高', '最低', '收盘']].std().mean(), 3)
    print(f'全部数据，{days}天平均滚动价格标准差为：{std_roll_values}，全部数据平均价格标准差为：{std_values}')
    return data
...
def cal_ATR(data, days):
    ...
    result = round(data['ATR'].mean(), 3)
    print(f'全部数据，{days}天平均滚动ATR为：{result}')
    return result
