...
def calculate_moving_averages(data, windows=[20, 60, 120]):
    ...
    # print('result_kv', result_kv['MA_20'])
    # print(f'均线计算结果:{result}')
    return result_kv
...
def plot_moving_averages(data, ticker):
    ...
    # plt.plot(data['收盘'], label='Close Price', alpha=0.5)
    
    # 绘制各条均线
    colors = ['orange', 'green', 'red']
    i = 0 
    for key, value in data.items():
        plt.plot(value, label=key, color=colors[i], linewidth=1.5)
        i += 1

    plt.title(f'{ticker} 股票价格与均线系统')
    plt.legend(loc='best')
    plt.show()
