...
if __name__ == '__main__':
    ...
    for symbol in stock_symbol_list:
        ...
        print(f"今日：{data.iloc[-1]['日期']}，{symbol}，收盘价为：{data.iloc[-1]['收盘']}，最高价为：{data.iloc[-1]['最高']}，最低价为：{data.iloc[-1]['最低']}，J值是否满足小于-5：{J_boolean}，洗盘条件是否满足单针下20：{SHAKEOUT_boolean}，近{days}天收盘价格是否满足大于bbi：{BBI_boolean}，MACD是否满足短期趋势强且在水上：{MACD_boolean}")
