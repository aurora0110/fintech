...
def download_daily_trade_volume(symbol, retry):   
    ...
    for _ in range(retry):
        try:
            ...
            stock_data = stock_zh_a_spot_df[stock_zh_a_spot_df["代码"] == f"sh{symbol}" if symbol.startswith("6") else f"sz{symbol}"]
            print(f"当前{symbol}总市值为{stock_data.iloc[0]["总市值"]}")
            # 自动识别市场前缀
            ...
            return pd.concat([df, stock_data.iloc[0]["总市值"]], axis=1)
        except Exception as e:
