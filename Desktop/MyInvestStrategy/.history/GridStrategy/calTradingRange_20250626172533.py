...
def cal_volatility(data, days):
    ...
    std_values_high = round(data[['最高', '最低', '收盘']].std().mean(), 4)
    std_values_low = round(data[['最高', '最低', '收盘']].std().mean(), 4)
    std_values_close = round(data[['最高', '最低', '收盘']].std().mean(), 4)

    print(f'全部数据，{days}个交易日内，平均滚动价格标准差为：{std_roll_values}，\n全部数据平均价格标准差为：{std_values}')
    return std_roll_values, std_values
