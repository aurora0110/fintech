...
def backTest(file, amount, windows, total_shares, each_buy_shares, start_date, end_date, backtest_log_path):
    ...
    print('=====================',len(sell_list), len(buy_list))
    with open(backtest_log_path, 'a') as f:
        f.write(f'{start_date}到{end_date}平均年化收益: {round(avg_profit * 100, 3)}%，平均持有时长为：{round(sum(holddays_list) / len(holddays_list),1)}天，年平均买卖操作次数为：{(len(sell_list) + len(buy_list)) / int(end_year - start_year)}次\n\n')
    return result
