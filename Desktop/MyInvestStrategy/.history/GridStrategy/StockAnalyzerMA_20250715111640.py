...
class StockAnalyzer:
    ...
    def plot_all(self):
        df_ma = self.data_ma.copy()
        df_bbi = self.data_bbi.copy()
        df_kdj = self.data_kdj.copy()
        df_macd = self.data_macd.copy()
        df_shakeout = self.data_shakeout.copy()
        df_price = self.data_price.copy()

        # x_axis 是形如 ['2020-01-01', ...] 的字符串列表
        x_axis = pd.to_datetime(self.data_ma['date'])
