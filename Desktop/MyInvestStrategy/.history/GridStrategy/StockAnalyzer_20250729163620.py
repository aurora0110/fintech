...
class StockMonitor:
    ...
    def twin_tails_monitor(self):
        ...
        recent_df_30 = df.tail(period_days_30).copy()
        recent_df_60 = df.tail(period_days_60).copy()

        if recent_df_30.shape[0] < period_days_30:
            print("数据不足指定周期，跳过检查。")
            return False
        ...
        # 最高价和最低价
        highest_price_30 = recent_df_30['最高价'].max()
        lowest_price_30 = recent_df_30['最低价'].min()
        return 
