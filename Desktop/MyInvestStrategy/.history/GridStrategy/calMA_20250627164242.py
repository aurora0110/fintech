...
def plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, ticker, windows):
    ...
    for x,y in zip(x_axis, data_kdj['J']):
        if y <= -5:
            ...
            plt.annotate(f'({x:.1f},{y:.1f})', xy=(x,y), xytext=(x+0.5,y+2), arrowprops=dict(facecolor='black', shrink=0.05), bbox=dict(boxstyle="round,pad=0.3", fc="yellow",alpha=0.5))
        elif y >= 5:
            plt.scatter(x,y,color='green',s=150,zorder=3)
