...
if __name__ == '__main__':
    ...
    for symbol in symbol_list:
        ...
        print(f'{data.iloc[-1]["日期"]}至{data.iloc[0]["日期"]}，j值小于等于0有{len(data_kdj[0])}个，j值小于等于-5有{len(data_kdj[1])}个，j值小于等于-8有{len(data_kdj[2])}个，j值小于等于-10有{len(data_kdj[3])}个，j值小于等于-15有{len(data_kdj[4])}个，j值小于等于-20有{len(data_kdj[5])}个，j值小于等于-25有{len(data_kdj[6])}个，j值大于等于80有{len(data_kdj[7])}个，j值大于等于90有{len(data_kdj[8])}个，j值大于等于100有{len(data_kdj[9])}个，j值大于等于110有{len(data_kdj[10])}个，j值大于等于120有{len(data_kdj[11])}个')
        ...
        cm.plot_all(data_ma, data_bbi, data_price, symbol, windows)

        # 计算MACD
        data_macd = calMACD.cal_macd(data, "2024-06-12", "2025-06-12")
        cm.plot_macd(data_macd, symbol)
        print('***********' * 10)
