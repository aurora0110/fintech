...
def plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, ticker, windows):
    ...
    x_axis_datetime = pd.to_datetime(x_axis)
    highlight_mask = data_kdj['J'] <= -5
    highlight_x = x_axis_datetime[highlight_mask]
    highlight_y = data_kdj['J'][highlight_mask]
    plt.scatter(highlight_x,highlight_y,color='red',s=150,zorder=3)

    for x,y in zip(x_axis_datetime, data_kdj['J']):
        if y <= -5:
            plt.annotate(f'{x.strftime("%Y-%m-%d")}\n{y:.1f}', xy=(x,y), xytext=(x+0.5,y+2), arrowprops=dict(facecolor='black', shrink=0.05), bbox=dict(boxstyle="round,pad=0.3", fc="yellow",alpha=0.5))
