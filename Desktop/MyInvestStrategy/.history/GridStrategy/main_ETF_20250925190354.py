...
if __name__ == '__main__':
    ...
    for symbol in tqdm(all_etf_codes):
        ...
        if J_boolean and MACD_boolean:
            select_list_JM.append([symbol, pd_dict[symbol]])
        
        if data_kdj['J'].iloc[-1] >= 90:
            select_list_J_sell.append([symbol, pd_dict[symbol]])
        ...
        print("🐤" * 90)
    print("J值小于阈值的有：",select_list_J)
    print("J值大于90的有：", select_list_J_sell)

    for code, name in select_list_J:
        print(f"{code}，{name}")
    print("-----------------------------------上边是买入，下边是卖出-------------------------------")
    for code, name in select_list_J_sell:
        print(f"{code}，{name}")
