...
if __name__ == '__main__':
    ...
    for symbol in stock_symbol_list:
        ...
        with open(backtest_log_path_new, 'a') as f:
            ...
            f.write(f"❗️异常交易金额和交易量{BS_Vol_Price_Monitor}")
        print(f"⏰今日：{data.iloc[-1]['日期']}，{symbol}，收盘价为：{data.iloc[-1]['收盘']}，最高价为：{data.iloc[-1]['最高']}，最低价为：{data.iloc[-1]['最低']}，J值为：{round(data_kdj['J'].iloc[-1],3)}，MACD值为：{round(data_macd['DIF'].iloc[-1],3)}，单针下20短期指标为：{round(data_shakeout['短期'].iloc[-1],3)}，单针下20长期指标为：{round(data_shakeout['长期'].iloc[-1],3)}")
    ...
    print(f"买入信号✅STOCK当日满足J值小于{J_threshold}的有{len(stock_select_list_J)}个：{stock_select_list_J}，❗️持有且大于9️⃣0️⃣的有{len(stock_select_list_J_sell)}个：{stock_select_list_J_sell}，⬇️单针下20信号的有{len(stock_select_list_S)}个:{stock_select_list_S}，J值快速下降的有{len(stock_fast_down_j_list)}个：{stock_fast_down_j_list}",)
    ...
    print(f"异常信号❗️STOCK满足J值快速下降（3天内下降值>=60）的有{len(stock_fast_down_j_list)}个：{stock_fast_down_j_list}，满足连续2天出现洗盘信号的有{len(stock_2days_shakeout_list)}个：{stock_2days_shakeout_list}，满足10天出现3次洗盘信号的有{len(stock_5days_shakeout_list)}个：{stock_5days_shakeout_list}")
    print(f"卖出信号🥳STOCK出现异常成交量和成交额的有{len(stock_bs_vol_price_list)}个：{stock_bs_vol_price_list}")