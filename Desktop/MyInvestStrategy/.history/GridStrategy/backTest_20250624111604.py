...
def backTest(file_path, amount, windows, total_shares):
    ...
    for i in range(len(df)):
        ...
        if not recent_sell and shares_held >= 0 and len(buy_prices) > 0:
            ...
            if sell_shares > 0 and shares_held >= sell_shares and price > buy_prices[-1]:
                revenue = price * sell_shares
                cash += revenue
                shares_held -= sell_shares
                # 移除已卖出的买入价格记录，先进先出
                buy_prices.pop(0) if len(buy_prices) > 0 else None
                trade_history.append({'date': current_date, 'type': 'sell', 'shares': sell_shares, 'price': price, 'revenue':revenue, 'buy_prices':buy_prices})
