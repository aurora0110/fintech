...
if __name__ == '__main__':
    ...
    for symbol in etf_symbol_list:
        ...
        print(f"技术指标：J值小于-5：{J_boolean}，MACD指标：DIF水上：{MACD_boolean}，单针下20短期指标小于20且单针下20长期指标大于60：{SHAKEOUT_boolean}，最近连续{bbi_days}天的收盘价格大于bbi：{BBI_boolean}")
        print("***********" * 10)
    ...
    print("💗" * 10, "ETF 今日数据如下", "💗" * 10)
    print(f"ETF当前回测策略为：可投入金额为{amount}元，最小操作间隔为{ineterval_days}天，计划操作手数为{total_shares}手")
    print(f"ETF回测策略年化收益大于10%有{len(well_list)}个：{well_list}，分别为：{well_list}")
    print(f"ETF回测策略年化收益小于10%有{len(ordinary_list)}个：{ordinary_list}，分别为：{ordinary_list}")
    print(f"ETF当日满足J值小于-5的ETF有{len(select_list_J)}个：{select_list_J}")
    print(f"ETF当日满足J值小于-5的ETF,且MACD水上的有{len(select_list_JM)}个：{select_list_JM}")
    print(f"ETF当日满足J值小于-5，单针下20短期指标小于20且单针下20长期指标大于60的ETF有{len(select_list_JS)}个：{select_list_JS}")
    print(f"ETF当日满足J值小于-5，单针下20短期指标小于20且单针下20长期指标大于60，最近连续10天的收盘价格大于bbi的ETF有{len(select_list_JSBBI)}个：{select_list_JSBBI}")
    print("💗" * 10, "STOCK 今日数据如下", "💗" * 10)
    print(f"STOCK当前回测策略为：可投入金额为{amount}元，最小操作间隔为{ineterval_days}天，计划操作手数为{total_shares}手")
    print(f"STOCK回测策略年化收益大于10%有{len(stock_well_list)}个：{stock_well_list}，分别为：{stock_well_list}")
    print(f"STOCK回测策略年化收益小于10%有{len(stock_ordinary_list)}个：{stock_ordinary_list}，分别为：{stock_ordinary_list}")
    print(f"STOCK当日满足J值小于-5的有{len(stock_select_list_J)}个：{stock_select_list_J}")
    print(f"STOCK当日满足J值小于-5的,且MACD水上的有{len(stock_select_list_JM)}个：{stock_select_list_JM}")
    print(f"STOCK当日满足J值小于-5，单针下20短期指标小于20且单针下20长期指标大于60的有{len(stock_select_list_JS)}个：{stock_select_list_JS}")
    print(f"STOCK当日满足J值小于-5，单针下20短期指标小于20且单针下20长期指标大于60，最近连续10天的收盘价格大于bbi的有{len(stock_select_list_JSBBI)}个：{stock_select_list_JSBBI}")
