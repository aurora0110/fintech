...
def plot_all(data_ma, data_bbi, data_price, data_macd, data_kdj, ticker, windows):
    ...
    x_axis_d = pd.to_datetime(x_axis)
    x_axis_d = x_axis_d.to_pydatetime()  # 关键修正

    highlight_mask = data_kdj['J'] <= -5
    highlight_dates = x_axis_d[highlight_mask]
    highlight_prices = data_kdj['J'][highlight_mask]
    for date, price in zip(highlight_dates, highlight_prices):
        ax5.annotate(f'{price}', xy=(date, price), xytext=(date, price+0.1),
