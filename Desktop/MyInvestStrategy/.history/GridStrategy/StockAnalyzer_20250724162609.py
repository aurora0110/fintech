...
class StockMonitor:
    ...
    def bs_abnormal_monitor(self):
        ...
        * 开盘收盘价格是从000001.csv（历史价格）文件中获取的，开盘收盘总价和总量是从000001_volume.csv（只有每天最新的价格）文件中获取的，如果想看历史数据可以去通达信导出
        ...
        print(f"当日开盘价：{open_price}，收盘价：{close_price}， {'📈' if close_price > open_price else '📉'}， 卖出总额：{sellprice_amount}，买入总额：{buyprice_amount}，净买入总额：{buyprice_amount-sellprice_amount}，卖出总量：{sellvolume_amount}，买入总量：{buyvolume_amount}，净买入总量：{buyvolume_amount-sellvolume_amount}")
