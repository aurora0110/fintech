...
def plot_moving_averages(data, ticker, windows, colors):
    ...
    for key, value in data.items():
        if i < len(colors):
            y_axis.append(value.tolist())
            i += 1
        else:
            pass
    
    line_nums = len(y_axis)

    for i in range(line_nums):
        plt.plot(x_axis, y_axis[i], label=key, color=colors[i], linewidth=1.5)
    ...
    ystep = 100
    selected_yticks = y_axis[::ystep]
    plt.xticks(selected_ticks, rotation=45)
    plt.yticks(selected_yticks, rotation=45)
    plt.xlabel('date')
    plt.ylabel('price')
