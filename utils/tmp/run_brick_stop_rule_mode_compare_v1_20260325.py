...
base = load_module(BASE_SCRIPT_PATH, "brick_stop_rule_compare_base_v1")
...
def stop_price_from_rule(entry_row: pd.Series, rule: str) -> float | None:
    ...
    if rule == "entry_close":
        return entry_close
    if rule == "entry_close_x_0.99":
        return entry_close * 0.99
    if rule == "entry_low":
        return entry_low
    if rule == "entry_low_x_0.99":
        return entry_low * 0.99
...
def simulate_one_trade(
    ...
    if stop_price is None or not np.isfinite(stop_price) or stop_price <= 0:
        return SimResult(trade=None, skipped=True, skip_reason=f"invalid_stop_price_{stop_rule}")
    ...
    return SimResult(
        trade={
            ...
            "stop_rule": stop_rule,
            "strategy_key": f"{execution_mode}|{stop_rule}",
        }
    )
...
def run_compare(file_limit: int, output_dir: Path, max_workers: int) -> None:
    ...
    candidates, coverage = load_candidates(file_limit=file_limit)
    ...
    if candidates.empty:
        raise RuntimeError("候选为空，无法进行止损规则对比实验")

    min5_registry = {str(code): MIN5_DIR / f"{code}.txt" for code in coverage.loc[coverage["has_5min"].astype(bool), "code"].astype(str)}
    update_progress(
        output_dir,
        "data_ready",
        candidate_count=len(candidates),
        code_count=int(candidates["code"].nunique()),
        min5_count=len(min5_registry),
        strategy_count=len(EXECUTION_MODES) * len(STOP_RULES),
    )
    ...
    total_jobs = len(candidates) * len(EXECUTION_MODES) * len(STOP_RULES)
    completed_codes = 0
    ...
    trades = pd.DataFrame(trade_rows).sort_values(["stop_rule", "execution_mode", "signal_date", "code"]).reset_index(drop=True) if trade_rows else pd.DataFrame()
    skipped = pd.DataFrame(skipped_rows).sort_values(["stop_rule", "mode", "signal_date", "code"], na_position="last").reset_index(drop=True) if skipped_rows else pd.DataFrame()
    ...
    gap_rows = []
    best_equity_paths = []
    for (stop_rule, mode), mode_df in trades.groupby(["stop_rule", "execution_mode"]):
        strategy = f"{mode}|{stop_rule}"
        summary = base.summarize_trades(mode_df, strategy)
        summary["mode"] = mode
        summary["stop_rule"] = stop_rule
        ...
        equity = base._portfolio_curve(mode_df)
        slug = strategy.replace("|", "__")
        equity_path = output_dir / f"equity_{slug}.csv"
        equity.to_csv(equity_path, index=False, encoding="utf-8-sig")
        best_equity_paths.append(str(equity_path))
        for gap_group, gap_df in mode_df.groupby("gap_group"):
            row = base.summarize_trades(gap_df, f"{strategy}|{gap_group}")
            row["mode"] = mode
    ...
    gap_df = pd.DataFrame(gap_rows).sort_values(["stop_rule", "mode", "gap_group"]) if gap_rows else pd.DataFrame()
    ...
    best_by_stop_df = pd.DataFrame(best_by_stop)
    best_by_stop_df.to_csv(output_dir / "best_by_stop_rule.csv", index=False, encoding="utf-8-sig")

    best_by_mode = []
    for mode, g in summary_df.groupby("mode", sort=True):
        best_by_mode.append(g.iloc[0].to_dict())
    best_by_mode_df = pd.DataFrame(best_by_mode)
    best_by_mode_df.to_csv(output_dir / "best_by_mode.csv", index=False, encoding="utf-8-sig")
    ...
    summary_json = {
        ...
        "equity_files": best_equity_paths,
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="BRICK 止损基准与执行方式对比实验（日线+5分钟线）")
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else RESULT_ROOT / f"brick_stop_rule_mode_compare_v1_{args.mode}_{timestamp}"
