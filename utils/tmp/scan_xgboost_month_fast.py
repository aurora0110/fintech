...
def check_single(file_path: str) -> tuple:
    if not XGBOOST_AVAILABLE:
        return None, None
    ...
    code_match = re.search(r'(\d{6})', Path(file_path).stem)
    code = code_match.group(1) if code_match else Path(file_path).stem
    return code, {
...
def scan_date_parallel(date_str: str, workers: int = 4) -> List[Dict]:
    data_dir = Path(f"/Users/lidongyang/Desktop/Qstrategy/data/{date_str}/normal")
    if not data_dir.exists():
        return []
    
    file_paths = [str(fp) for fp in data_dir.glob('*.txt')]
    results = []
    
    with ProcessPoolExecutor(max_workers=workers) as executor:
        for code, data in executor.map(check_single, file_paths, chunksize=50):
            if code is not None:
                results.append({
                    'date': date_str,
                    'code': code,
...
def main():
    ...
    for date_str in dates:
        ...
        results = scan_date_parallel(date_str, workers)
