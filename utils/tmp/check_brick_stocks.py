...
def main():
    ...
    for code, market in brick_stocks:
        ...
        print(f"\n{code} ({row.get('name', 'N/A')}):")
        print(f"  trend_line: {row['trend_line']:.4f}")
        print(f"  long_line: {row['long_line']:.4f}")
        print(f"  trend_line > long_line: {row['trend_line'] > row['long_line']}")
        print(f"  brick_red_len: {row['brick_red_len']:.4f}")
        print(f"  prev_brick_green_len: {prev_row['brick_green_len']:.4f}")
        if prev_row['brick_green_len'] > 0:
            rebound = row['brick_red_len'] / prev_row['brick_green_len']
            print(f"  rebound_ratio: {rebound:.4f}")
            print(f"  rebound_ratio > 0.66: {rebound > 0.66}")
        
        features = extract_features(df, signal_idx)
        if features is None:
            print("  不满足XGBoost brick条件!")
            continue
