...
BASELINE_SUMMARY = ROOT / "results" / "brick_case_rank_atr_tp_search_v1_full_20260328_r1" / "summary.json"
...
base = load_module(BASE_SCRIPT, "brick_case_rank_histvol_tp_base")
...
def _compute_histvol_columns(daily_df: pd.DataFrame, windows: list[int]) -> pd.DataFrame:
    df = daily_df.sort_values("date").copy()
    close = pd.to_numeric(df["close"], errors="coerce")
    ret = close.pct_change()
    for window in windows:
        vol = ret.rolling(window, min_periods=window).std(ddof=0)
        df[f"histvol_{window}"] = vol
    return df
    ...
    for vol_window in VOL_WINDOWS:
        for k in k_values:
            for hold_days in HOLD_DAYS:
                profiles.append(
                    {
                        "name": f"histvol_tp_w{vol_window}_k{k:.4f}_h{hold_days}",
                        "family": "histvol_fixed_tp",
...
def _histvol_pct_for_signal(signal_row: pd.Series, vol_window: int) -> float | None:
    vol_value = float(signal_row.get(f"histvol_{vol_window}", np.nan))
    if not np.isfinite(vol_value) or vol_value <= 0:
        return None
...
def simulate_code_bundle(payload: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    daily_df = _compute_histvol_columns(daily_df, VOL_WINDOWS)
    ...
    for item in rows:
        ...
        for profile in profiles:
            vol_pct = _histvol_pct_for_signal(signal_row, int(profile["vol_window"]))
            if vol_pct is None:
                skipped.append(
                    {
                        "code": code,
            ...
            trade.update(
                {
                    "profile_name": profile["name"],
                    "profile_family": "histvol_fixed_tp",
                    "vol_window": int(profile["vol_window"]),
                    "k_value": float(profile["k_value"]),
                    "histvol_pct": float(vol_pct),
...
def _summarize_signal_basket(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, g in trades.groupby(group_cols, sort=True):
        ...
        strategy = f"case_rank_histvol|{stop_base}|{stop_exec_mode}|{profile_name}"
        ...
        row.update(
            {
                ...
                "avg_histvol_pct": float(pd.to_numeric(g["histvol_pct"], errors="coerce").mean()),
                "stop_base": stop_base,
...
def _summarize_account(trades: pd.DataFrame, result_dir: Path, max_workers: int) -> pd.DataFrame:
    ...
    extra = (
        ...
        .agg(
            ...
            avg_histvol_pct=("histvol_pct", "mean"),
        )
        .reset_index()
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="历史波动率动态止盈搜索：固定买点/止损/执行方式，只搜索窗口 × K × 持有天数，K 自动扩边直到不贴边")
    ...
    parser.add_argument("--start-k-min", type=float, default=1.0)
    parser.add_argument("--start-k-max", type=float, default=4.0)
    parser.add_argument("--k-step", type=float, default=0.5)
    parser.add_argument("--lower-cap", type=float, default=0.5)
...
def main() -> None:
    ...
    output_root = Path(args.output_root) if args.output_root else RESULT_ROOT / f"brick_case_rank_histvol_tp_search_v1_{args.mode}_{timestamp}"
    ...
    try:
        ...
        final = {
            ...
            "baseline_best_atr_tp": _load_baseline_best(),
            "rounds": rounds,
