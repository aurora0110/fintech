...
BASELINE_SUMMARY = ROOT / "results" / "brick_case_rank_h3_fixed_tp_extend_v1_full_20260327_r1" / "summary.json"
...
base = load_module(BASE_SCRIPT, "brick_case_rank_atr_tp_base")
    ...
    for atr_period in ATR_PERIODS:
        for k in k_values:
            for hold_days in HOLD_DAYS:
                profiles.append(
                    {
                        "name": f"atr_tp_p{atr_period}_k{k:.4f}_h{hold_days}",
                        "family": "atr_fixed_tp",
...
def _summarize_signal_basket(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for keys, g in trades.groupby(group_cols, sort=True):
        ...
        strategy = f"case_rank_atr|{stop_base}|{stop_exec_mode}|{profile_name}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ATR 动态止盈搜索：固定买点/止损/执行方式，只搜索 ATR 周期 × K × 持有天数，K 自动扩边直到不贴边")
...
def main() -> None:
    ...
    output_root = Path(args.output_root) if args.output_root else RESULT_ROOT / f"brick_case_rank_atr_tp_search_v1_{args.mode}_{timestamp}"
    ...
    try:
        ...
        final = {
            ...
            "baseline_best_fixed_tp": _load_baseline_best(),
            "rounds": rounds,
