...
base = load_module(BASE_SCRIPT, "brick_case_rank_h3_fixed_tp_extend_base")
...
def simulate_code_bundle_fixed(payload: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def run_search_fixed(
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_map = {executor.submit(simulate_code_bundle_fixed, payload): payload["code"] for payload in grouped_payloads}
        completed = 0
    ...
    signal_summary = signal_summary[signal_summary["profile_family"] == "fixed_tp"].copy()
    ...
    account_summary = account_summary[account_summary["profile_family"] == "fixed_tp"].copy()
    account_summary.to_csv(result_dir / "account_summary.csv", index=False, encoding="utf-8-sig")

    summary = {
        "mode": "fixed_tp_h3_extend",
        "max_tp": max_tp,
        "profile_count": len(profiles),
...
def best_tp_is_edge(summary: dict[str, Any], current_max_tp: float) -> tuple[bool, dict[str, Any]]:
    best = summary.get("best_account_profile", {}) or {}
    best_tp = float(best.get("tp_pct", -1))
    return abs(best_tp - current_max_tp) < 1e-12, best


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="3日窗口固定止盈专用扩边搜索，直到最优止盈不再贴上边界")
...
def main() -> None:
    ...
    output_root = Path(args.output_root) if args.output_root else RESULT_ROOT / f"brick_case_rank_h3_fixed_tp_extend_v1_{args.mode}_{timestamp}"
    ...
    try:
        while True:
            ...
            edge, best = best_tp_is_edge(summary, current_max_tp)
            rounds.append(
                {
                    "result_dir": str(round_dir),
                    "max_tp": current_max_tp,
                    "best_account_profile": best,
                    "best_tp_on_upper_edge": edge,
                }
            )
            if not edge:
                break
        ...
        final = {
            ...
            "stopped_because": "interior_best_found" if rounds and not rounds[-1]["best_tp_on_upper_edge"] else "hit_max_cap_or_no_rounds",
            "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        }
