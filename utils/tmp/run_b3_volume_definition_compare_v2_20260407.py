...
b3filter = load_module(ROOT / "utils" / "B3filter.py", "b3_voldef_b3")
technical = load_module(ROOT / "utils" / "technical_indicators.py", "b3_voldef_technical")
...
def analyze_one_file(file_path_str: str) -> list[dict[str, Any]]:
    ...
    sig_no_req = base_signal
    sig_expand_prev = base_signal & (vol > vol_prev)
    sig_shrink_prev = base_signal & (vol < vol_prev)
    sig_shrink_ma5 = base_signal & (vol < vol_ma5)
    sig_shrink_both = base_signal & (vol < vol_prev) & (vol < vol_ma5)

    group_defs = [
        ("no_requirement", sig_no_req),
        ("volume_expand_prev", sig_expand_prev),
        ("volume_shrink_prev", sig_shrink_prev),
        ("volume_shrink_ma5", sig_shrink_ma5),
        ("volume_shrink_prev_and_ma5", sig_shrink_both),
    ]

    rows: list[dict[str, Any]] = []
    for idx, row in x.iterrows():
        for group_name, sig in group_defs:
            ...
            item: dict[str, Any] = {
                ...
                "volume": float(vol.iloc[idx]) if np.isfinite(vol.iloc[idx]) else float("nan"),
                "vol_prev": float(vol_prev.iloc[idx]) if idx < len(vol_prev) and np.isfinite(vol_prev.iloc[idx]) else float("nan"),
                "vol_ma5": float(vol_ma5.iloc[idx]) if idx < len(vol_ma5) and np.isfinite(vol_ma5.iloc[idx]) else float("nan"),
...
def summarize_group(df: pd.DataFrame) -> pd.DataFrame:
    ...
    order = {
        "no_requirement": 0,
        "volume_expand_prev": 1,
        "volume_shrink_prev": 2,
        "volume_shrink_ma5": 3,
        "volume_shrink_prev_and_ma5": 4,
    }
...
def run(cfg: AnalysisConfig) -> None:
    ...
    if signal_df.empty:
        raise RuntimeError("最近两年窗口内未找到任何可比较的 B3 基础信号")
    ...
    signal_df.to_csv(cfg.output_dir / "b3_volume_definition_signal_detail.csv", index=False, encoding="utf-8-sig")

    summary_df = summarize_group(signal_df)
    summary_df.to_csv(cfg.output_dir / "b3_volume_definition_summary.csv", index=False, encoding="utf-8-sig")

    summary = {
        "assumptions": {
            "data_dir": str(cfg.data_dir),
            "backtest_window": [str(BACKTEST_START.date()), str(BACKTEST_END.date())],
            "signal_definition": "current_B3filter_base_signal_with_alternative_volume_definitions",
            "volume_groups": {
                "no_requirement": "base B3 structure, no signal-day volume rule",
                "volume_expand_prev": "signal-day volume > previous day volume",
                "volume_shrink_prev": "signal-day volume < previous day volume",
                "volume_shrink_ma5": "signal-day volume < 5-day average volume",
                "volume_shrink_prev_and_ma5": "signal-day volume < previous day and < 5-day average volume",
            },
...
def main() -> None:
    ...
    cfg = AnalysisConfig(
        output_dir=OUTPUT_ROOT / f"b3_volume_definition_compare_v2_{ts}",
        data_dir=DATA_DIR,
