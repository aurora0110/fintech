...
def _build_signal_rows(file_path_str: str) -> list[dict]:
    ...
    full_df = _prepare_df(str(file_path))
    if full_df is None or full_df.empty:
        return []

    weekly_map = b1filter.map_weekly_screen_to_daily_df(full_df)
    full_df = full_df.merge(weekly_map, on="日期", how="left")
    full_df["weekly_ok"] = full_df["weekly_ok"].fillna(False).astype(bool)
    full_df = _calc_short_long(full_df)
    vol_ma5 = pd.to_numeric(full_df["成交量"], errors="coerce").rolling(5, min_periods=5).mean()
    code = _stock_code(file_path)

    rows: list[dict] = []
    date_mask = (full_df["日期"] >= START_DATE) & (full_df["日期"] <= END_DATE)
    valid_indices = full_df.index[date_mask].tolist()
    for i in valid_indices:
        if i >= len(full_df) - 1:
            continue
        today = full_df.iloc[i]
        ...
        entry_idx = i + 1
        entry_date = pd.Timestamp(full_df["日期"].iloc[entry_idx])
        if entry_date < START_DATE or entry_date > END_DATE:
            continue
        entry_open = float(full_df["开盘"].iloc[entry_idx])
...
def main() -> None:
    ...
    result_dir = RESULTS_DIR / f"pin_longterm80_vs_hold3_2y_v1_{ts}"
