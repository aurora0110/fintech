...
DEFAULT_RESULT_DIR = ROOT / "results" / f"b1_txt_template_signal_v1_{RUN_TS}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="B1 文本正例模板相似度定向优化")
...
def compute_template_scores(candidate_df: pd.DataFrame, pos_df: pd.DataFrame, neg_df: pd.DataFrame) -> pd.DataFrame:
    x = candidate_df.copy()
    template_rows = pos_df[pos_df["split"] == "research"].to_dict("records")
    hard_neg = neg_df[neg_df["split"] == "research"].to_dict("records")
    ...
    for method, rep in TARGET_REP_METHODS:
        ...
        if hard_neg:
            neg_tpl = stack_rep(hard_neg, rep)
            neg_col = f"neg_{method}_{rep}"
            ...
            x[f"contrast_{method}_{rep}"] = x[f"rank_{score_col}"] - 0.6 * x[f"rank_{neg_col}"]
    ...
    contrast_cols = [c for c in ["contrast_corr_close_vol_concat", "contrast_cosine_close_vol_concat", "contrast_weighted_corr_close_vol_concat", "contrast_lag_corr_close_vol_concat"] if c in x.columns]
    if contrast_cols:
        x["template_contrast_score"] = np.mean(np.column_stack([x[c] for c in contrast_cols]), axis=1)
    else:
        x["template_contrast_score"] = x["template_similarity_score"]
...
def build_ml_scores(candidate_df: pd.DataFrame, pos_df: pd.DataFrame, neg_df: pd.DataFrame) -> pd.DataFrame:
    x = candidate_df.copy()
    ...
    auto_neg = auto_neg.sort_values(["ret_20d", "min_close_ret_30"]).head(max(80, len(research_pos) * 4))
    ...
    if not research_neg.empty:
        ...
        train_rows.append(tmp)
    if not auto_neg.empty:
        train_rows.append(auto_neg)
    if not train_rows:
        x["template_ml_score"] = 0.5
        return x
    ...
    if HAS_SKLEARN:
        try:
            ...
            lr.fit(X, y)
            x["template_lr_score"] = lr.predict_proba(X_all)[:, 1]
            score_cols.append("template_lr_score")
        except Exception:
            pass
        try:
            ...
            et.fit(X, y)
            x["template_et_score"] = et.predict_proba(X_all)[:, 1]
            score_cols.append("template_et_score")
        except Exception:
            pass
    if HAS_LGB:
        try:
            ...
            clf.fit(X, y)
            x["template_lgb_score"] = clf.predict_proba(X_all)[:, 1]
            score_cols.append("template_lgb_score")
        except Exception:
            pass
    if HAS_XGB:
        try:
            ...
            clf.fit(X, y)
            x["template_xgb_score"] = clf.predict_proba(X_all)[:, 1]
            score_cols.append("template_xgb_score")
        except Exception:
            pass
    ...
    if score_cols:
        ...
        x["template_ml_score"] = np.mean(np.column_stack([x[f"rank_{col}"] for col in score_cols]), axis=1)
    else:
        x["template_ml_score"] = 0.5
    return x
...
def add_fusion_scores(candidate_df: pd.DataFrame) -> pd.DataFrame:
    x = candidate_df.copy()
    x["template_factor_fusion_score"] = (
    ...
    x["template_full_fusion_score"] = (
        0.35 * ecdf_score(x.loc[x["split"] == "research", "discovery_factor_score"], x["discovery_factor_score"])
        + 0.25 * x["template_similarity_score"]
        + 0.20 * x["template_ml_score"]
        + 0.20 * x["template_confirm_score"]
    )
    return x
...
def evaluate_strategies(df: pd.DataFrame, topn_list: Iterable[int]) -> pd.DataFrame:
    ...
    strategy_specs = [
        ...
        ("factor", "champion_on_txt_pool", "pool_txt_dual", "discovery_factor_score"),
        ("similarity", "template_similarity", "pool_txt_dual", "template_similarity_score"),
        ("contrast", "template_contrast", "pool_txt_dual", "template_contrast_score"),
        ("ml", "template_ml", "pool_txt_confirmed", "template_ml_score"),
        ("fusion", "template_factor_fusion", "pool_txt_confirmed", "template_factor_fusion_score"),
        ("fusion", "template_full_fusion", "pool_txt_confirmed", "template_full_fusion_score"),
    ]
...
def main() -> None:
    ...
    candidate_df = compute_template_scores(candidate_df, pos_df, neg_df)
    candidate_df = build_ml_scores(candidate_df, pos_df, neg_df)
