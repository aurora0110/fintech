...
DEFAULT_RESULT_DIR = ROOT / "results" / f"b1_txt_template_signal_v2_{RUN_TS}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="B1 文本正例模板相似度 + 近似反例增强优化")
...
def build_auto_hard_negatives(candidate_df: pd.DataFrame, pos_df: pd.DataFrame, neg_df: pd.DataFrame) -> pd.DataFrame:
    ...
    pool = candidate_df[
        ...
        & candidate_df["pool_txt_dual"].fillna(False)
...
def evaluate_strategies(df: pd.DataFrame, topn_list: Iterable[int]) -> pd.DataFrame:
    ...
    strategy_specs = [
        ...
        ("contrast", "template_hard_contrast", "pool_txt_dual", "template_hard_contrast_score"),
        ("ml", "template_hard_ml", "pool_txt_confirmed", "template_hard_ml_score"),
        ("fusion", "template_hard_factor_fusion", "pool_txt_confirmed", "template_hard_factor_fusion_score"),
        ("fusion", "template_hard_full_fusion", "pool_txt_confirmed", "template_hard_full_fusion_score"),
    ]
...
def main() -> None:
    ...
    hard_neg_df = build_auto_hard_negatives(candidate_df, pos_df, neg_df)
