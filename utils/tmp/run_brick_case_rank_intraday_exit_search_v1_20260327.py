...
TP_LEVELS = [0.03, 0.035, 0.04, 0.045, 0.05, 0.055, 0.06, 0.065, 0.07, 0.075, 0.08]
WEAK_COUNTS = [2, 3, 4, 5, 6]
...
tp_mod = load_module(TP_SEARCH_PATH, "brick_case_rank_intraday_tp_mod")
real_account = load_module(REAL_ACCOUNT_PATH, "brick_case_rank_intraday_real_account")
...
def _profile_defs() -> list[dict[str, Any]]:
    ...
    dynamic_exit_modes = [
        ("same_day_intraday_nextbar", "intraday"),
        ("same_day_intraday_nextbar_close", "intraday_close"),
    ...
    for tp_pct in TP_LEVELS:
        ...
        profiles.extend(
            [
                {"name": f"tp_intraday_{tag}", "family": "baseline", "tp_pct": tp_pct, "exit_mode": "same_day_intraday_nextbar"},
                {"name": f"tp_close_{tag}", "family": "baseline", "tp_pct": tp_pct, "exit_mode": "same_day_close"},
                {"name": f"tp_next_open_{tag}", "family": "baseline", "tp_pct": tp_pct, "exit_mode": "next_day_open"},
                {"name": f"tp_next_close_{tag}", "family": "baseline", "tp_pct": tp_pct, "exit_mode": "next_day_close"},
            ]
        )
        for weak_n in WEAK_COUNTS:
            for exit_mode, exit_tag in dynamic_exit_modes:
                profiles.append(
                    {
                        "name": f"profit_protect_{exit_tag}_{tag}_n{weak_n}",
                        "family": "profit_protect",
                        "tp_pct": tp_pct,
                        "exit_mode": exit_mode,
                        "weak_n": weak_n,
                    }
...
def _resolve_baseline_exit(
    ...
    if exit_mode == "next_day_close":
        next_date = _next_trade_date(daily_df, pd.Timestamp(trigger_date))
        if next_date is None:
            return pd.Timestamp(trigger_date), float(day_daily_row["close"]), "fallback_same_day_close"
        return _daily_exit(daily_df, next_date, "close")
    if exit_mode == "same_day_intraday_nextbar":
        ...
        return pd.Timestamp(trigger_date), float(day_daily_row["close"]), "fallback_same_day_close"
    if exit_mode == "same_day_intraday_nextbar_close":
        if trigger_dt is not None and not day_min.empty:
...
def simulate_one_trade_profile(
    ...
    stop_base_price = tp_mod.hybrid.resolve_stop_base(entry_open, entry_close, entry_low, stop_base)
    stop_price = float(stop_base_price) * stop_multiplier
    tp_price = entry_open * (1.0 + tp_pct)
    trade_window_dates, eligible_dates = _build_trade_window(daily_df, entry_date, max_hold_days)
    ...
    for d in eligible_dates:
        ...
        if day_min.empty:
            ...
            if (not armed) and day_high >= tp_price:
                ...
                trigger_source = "daily_fallback"
                if profile["family"] == "baseline":
                    exit_date, exit_price, mode_reason = _resolve_baseline_exit(
                        profile["exit_mode"],
                        pd.Timestamp(d),
                        None,
                        day_daily_row,
        ...
        for _, bar in day_min.iterrows():
            ...
            # Same-bar ambiguity remains unresolved by OHLC; keep conservative stop-first.
            ...
            if armed and profile["family"] == "profit_protect":
                prev_low = float(prev_bar["low"]) if prev_bar is not None and pd.notna(prev_bar["low"]) else np.nan
                is_weak = bool(np.isfinite(prev_low) and float(bar["close"]) < prev_low)
    ...
    return SimResult(
        trade={
            ...
            "strategy_key": f"{strategy_key_prefix}|{profile['name']}",
        }
    )
...
def _summarize_signal_basket(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for (profile_name, stop_base, stop_multiplier), g in trades.groupby(["profile_name", "stop_base", "stop_multiplier"], sort=True):
        strategy = f"case_rank|{stop_base}_x_{float(stop_multiplier):.4f}|{profile_name}"
    ...
    return pd.DataFrame(rows).sort_values(
        ["annual_return_signal_basket", "final_equity_signal_basket", "max_drawdown_signal_basket", "profit_factor"],
...
def _build_close_map_parallel(codes: list[str], result_dir: Path, max_workers: int) -> tuple[pd.DatetimeIndex, dict[str, pd.Series]]:
        ...
        for future in as_completed(future_map):
            ...
            if completed == 1 or completed % 50 == 0 or completed == total:
                update_progress(
                    result_dir,
                    "building_close_map",
                    done_codes=int(completed),
                    total_codes=int(total),
                    loaded_codes=int(len(relevant)),
                )
    ...
    for idx, (code, s) in enumerate(relevant.items(), start=1):
        ...
        if idx == 1 or idx % 50 == 0 or idx == len(relevant):
            update_progress(
                result_dir,
                "reindexing_close_map",
                done_series=int(idx),
                total_series=int(len(relevant)),
                market_days=int(len(market_dates)),
            )
...
def _summarize_account(trades: pd.DataFrame, result_dir: Path, max_workers: int) -> pd.DataFrame:
    ...
    def _simulate_intraday_account(trades_one: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, float]]:
        trades_one = trades_one.sort_values(["entry_date", "sort_score", "code"], ascending=[True, False, True]).reset_index(drop=True)
        entries_by_date = {
            d: g.sort_values(["sort_score", "code"], ascending=[False, True]).to_dict("records")
            for d, g in trades_one.groupby("entry_date")
        }
        exits_by_date = {d: g.to_dict("records") for d, g in trades_one.groupby("exit_date")}
    ...
    for (profile_name, stop_base, stop_multiplier), g in trades.groupby(["profile_name", "stop_base", "stop_multiplier"], sort=True):
        use = g.copy()
        use["strategy_key"] = f"case_rank|{stop_base}_x_{float(stop_multiplier):.4f}|{profile_name}"
        equity_df, executed_df, summary = _simulate_intraday_account(use)
        stem = f"{profile_name}|{stop_base}_x_{float(stop_multiplier):.4f}".replace(".", "p")
        ...
        row = {
            ...
            "stop_base": stop_base,
            "stop_multiplier": float(stop_multiplier),
            "stop_key": f"{stop_base}_x_{float(stop_multiplier):.4f}",
            **summary,
        }
        rows.append(row)
    return pd.DataFrame(rows).sort_values(
...
def run_search(
    ...
    trades = pd.DataFrame(trade_rows).sort_values(["profile_name", "stop_base", "stop_multiplier", "signal_date", "code"]).reset_index(drop=True) if trade_rows else pd.DataFrame()
    skipped = pd.DataFrame(skipped_rows).sort_values(["profile_name", "stop_base", "stop_multiplier", "signal_date", "code"], na_position="last").reset_index(drop=True) if skipped_rows else pd.DataFrame()
    ...
    summary = {
        "assumptions": {
            ...
            "same_bar_priority": "stop_first_on_same_bar_due_to_ohlc_limit",
            "event_priority": "first_come_by_bar_time",
            "trigger_fill_rule": "next_5min_open",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="固定 case_rank_lgbm_top20，搜索分钟级止盈止损矩阵")
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else RESULT_ROOT / f"brick_case_rank_intraday_exit_search_v1_{args.mode}_{timestamp}"
