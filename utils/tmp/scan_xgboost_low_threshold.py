...
def scan_file(file_path: str) -> List:
    ...
    return [1, code, name, date, round(proba, 4), round(row['close'], 2)]
...
def main():
    ...
    print("\n" + "="*80)
    print(f"XGBoost筛选结果 (阈值={THRESHOLD})")
    print("="*80)
    
    if not all_results:
        print(f"最近一个月无预测概率 >= {THRESHOLD} 的股票")
    else:
        print(f"共筛选出 {len(all_results)} 条数据:\n")
        print(f"{'日期':<12} {'代码':<8} {'名称':<10} {'预测概率':<10} {'收盘价':<10}")
        print("-"*60)
        for r in sorted(all_results, key=lambda x: (x[3], -x[4])):
            print(f"{r[3]:<12} {r[1]:<8} {r[2]:<10} {r[4]:<10.4f} {r[5]:<10.2f}")
