...
OUTPUT_DIR = os.environ.get(
    "B2_T1_OUTPUT_DIR", "/Users/lidongyang/Desktop/Qstrategy/results/b2_t1_green_optimization"
)
...
class Combo:
    ...
    def combo_name(self) -> str:
        return (
            ...
            f"__3日斜率_{self.趋势线3日斜率}__5日斜率_{self.趋势线5日斜率}"
            f"__斜率差扩大_{self.斜率差扩大}__收盘位置强_{self.收盘位置强}"
...
def build_feature_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    x["下影线占比"] = np.where(full_range > 0, lower_shadow / full_range, np.nan)
    x["实体占振幅"] = np.where(full_range > 0, real_body / full_range, np.nan)
    ...
    x["量能20日分位"] = x["volume"].rolling(20).rank(pct=True)
    ...
    x["趋势线3日斜率"] = np.where(x["趋势线"].shift(3) > 0, x["趋势线"] / x["趋势线"].shift(3) - 1.0, np.nan)
    x["趋势线5日斜率"] = np.where(x["趋势线"].shift(5) > 0, x["趋势线"] / x["趋势线"].shift(5) - 1.0, np.nan)
    x["多空线3日斜率"] = np.where(x["多空线"].shift(3) > 0, x["多空线"] / x["多空线"].shift(3) - 1.0, np.nan)
    x["多空线5日斜率"] = np.where(x["多空线"].shift(5) > 0, x["多空线"] / x["多空线"].shift(5) - 1.0, np.nan)
    x["3日斜率差"] = x["趋势线3日斜率"] - x["多空线3日斜率"]
    x["5日斜率差"] = x["趋势线5日斜率"] - x["多空线5日斜率"]
    x["距趋势线"] = safe_div(x["close"] - x["趋势线"], x["趋势线"], default=np.nan)
    x["距多空线"] = safe_div(x["close"] - x["多空线"], x["多空线"], default=np.nan)
    x["距20日高点"] = safe_div(x["close"], x["close"].rolling(20).max(), default=np.nan) - 1.0
    x["距20日低点"] = safe_div(x["close"], x["close"].rolling(20).min(), default=np.nan) - 1.0
    x["前5日涨幅"] = x["close"].shift(1) / x["close"].shift(6) - 1.0
    x["前10日涨幅"] = x["close"].shift(1) / x["close"].shift(11) - 1.0

    x["基础信号"] = x["trend_ok"] & (x["涨幅"] >= 0.04) & x["小上影"] & x["放量基础"] & x["J启动"]

    # ranking still favors lower heat + cleaner launch
...
def build_combos() -> List[Combo]:
    return [
        ...
        for 启动线, 放量强度, 斜率3, 斜率5, 斜率差, 收盘位置, 上影线, 近高点, 前10日不过热 in product(
            ["不限", "趋势线启动", "多空线启动", "双线启动"],
            ["仅基础放量", "温和放量", "中等放量"],
            [False, True],
            [False, True],
...
def simulate_trade(df: pd.DataFrame, signal_idx: int) -> Optional[dict]:
    ...
    return {
        ...
        "趋势线3日斜率": float(df.at[signal_idx, "趋势线3日斜率"]),
        "趋势线5日斜率": float(df.at[signal_idx, "趋势线5日斜率"]),
        "5日斜率差": float(df.at[signal_idx, "5日斜率差"]),
...
def summarize_combo(combo: Combo, trade_df: pd.DataFrame, portfolio_df: pd.DataFrame) -> dict:
    if trade_df.empty:
        ...
        row = {
            ...
            "平均趋势线5日斜率": float(trade_df["趋势线5日斜率"].mean()),
            "平均收盘位置": float(trade_df["收盘位置"].mean()),
        }
