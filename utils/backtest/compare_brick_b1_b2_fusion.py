...
from utils.backtest.run_b2_startup_experiment import load_one_csv as load_csv
from utils.backtest.run_b2_startup_experiment import build_feature_df as build_b2_features


DATA_DIR = os.environ.get("FUSION_DATA_DIR", "/Users/lidongyang/Desktop/Qstrategy/data/forward_data")
OUTPUT_DIR = os.environ.get("FUSION_OUTPUT_DIR", "/Users/lidongyang/Desktop/Qstrategy/results/brick_b1_b2_fusion")
MAX_FILES = int(os.environ.get("FUSION_MAX_FILES", "0"))
...
def build_base_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    x["J"] = 3 * x["K"] - 2 * x["D"]
    j_min = x["J"].rolling(30).min()
    j_max = x["J"].rolling(30).max()
    x["J_rank_30"] = (x["J"] - j_min) / (j_max - j_min + 1e-9)
...
def add_b1_signals(x: pd.DataFrame) -> pd.DataFrame:
    out = x.copy()
    out["b1_趋势线启动"] = (out["close"].shift(1) <= out["trend_line"].shift(1) * 1.01) & (out["close"] > out["trend_line"])
    out["b1_多空线启动"] = (out["close"].shift(1) <= out["long_line"].shift(1) * 1.01) & (out["close"] > out["long_line"])
    out["b1_双线启动"] = out["b1_趋势线启动"] & out["b1_多空线启动"]
    out["b1_任一线启动"] = out["b1_趋势线启动"] | out["b1_多空线启动"]
    out["b1_J上拐"] = (
        (out["J"] > out["J"].shift(1))
        & (out["J"].shift(1) < out["J"].shift(2))
        & (out["J"].shift(2) < out["J"].shift(3))
    )

    # 融合口径下放宽 B1：低J拐头 + 任一线/双线启动
    out["b1_low_20_anyline"] = out["trend_ok"] & (out["J"] < 20) & out["b1_J上拐"] & out["b1_任一线启动"]
    out["b1_low_30_anyline"] = out["trend_ok"] & (out["J"] < 30) & out["b1_J上拐"] & out["b1_任一线启动"]
    out["b1_rank_020_anyline"] = out["trend_ok"] & (out["J_rank_30"] <= 0.20) & out["b1_J上拐"] & out["b1_任一线启动"]
    out["b1_low_20_dualline"] = out["trend_ok"] & (out["J"] < 20) & out["b1_J上拐"] & out["b1_双线启动"]

    b1_quality = np.maximum(0.0, 1.0 - np.minimum(out["J"].fillna(100.0) / 20.0, 1.0))
    line_quality = np.maximum((out["close"] - np.maximum(out["trend_line"], out["long_line"])) / out["close"], 0.0)
    out["b1_sort_score"] = 0.6 * b1_quality + 0.4 * pd.Series(line_quality, index=out.index).fillna(0.0)
    return out
...
def load_feature_map(data_dir: str) -> Dict[str, pd.DataFrame]:
    ...
    for idx, file_name in enumerate(files, 1):
        ...
        base = add_b1_signals(base)
        b2_df = build_b2_features(df)
        base["b2_signal"] = b2_df["基础信号"] & b2_df["双线附近启动"]
        base["b2_sort_score"] = b2_df["sort_score"]
        feature_map[code] = base
...
def simulate_trade(df: pd.DataFrame, signal_idx: int, group_name: str) -> Optional[dict]:
    ...
    if "B2" in group_name and "B1" not in group_name:
        sort_score = float(df.at[signal_idx, "b2_sort_score"])
    elif "B1" in group_name and "B2" not in group_name:
        sort_score = float(df.at[signal_idx, "b1_sort_score"])
    elif "B1" in group_name and "B2" in group_name:
        sort_score = float(df.at[signal_idx, "brick_sort_score"] + df.at[signal_idx, "b1_sort_score"] + df.at[signal_idx, "b2_sort_score"])
    else:
        sort_score = float(df.at[signal_idx, "brick_sort_score"])
    return {
...
def summarize(group_name: str, trade_df: pd.DataFrame, portfolio_df: pd.DataFrame, b1_name: str) -> dict:
    row = {
...
def run_group(feature_map: Dict[str, pd.DataFrame], group_name: str, b1_col: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    trades = []
    for code, df in feature_map.items():
        brick_mask = df["brick_signal"].fillna(False)
        b1_mask = df[b1_col].fillna(False)
        b2_mask = df["b2_signal"].fillna(False)
        if group_name == "只满足砖型":
            mask = brick_mask
        elif group_name == "砖型+B1":
            mask = brick_mask & b1_mask
        elif group_name == "砖型+B2":
            mask = brick_mask & b2_mask
        else:
            mask = brick_mask & b1_mask & b2_mask
...
def main() -> None:
    ...
    candidates = [
        CandidateB1("J<20+任一线启动", "b1_low_20_anyline"),
        CandidateB1("J<30+任一线启动", "b1_low_30_anyline"),
        CandidateB1("J30分位<=0.20+任一线启动", "b1_rank_020_anyline"),
        CandidateB1("J<20+双线启动", "b1_low_20_dualline"),
    ]

    rows = []
    for candidate in candidates:
        for group_name in ["只满足砖型", "砖型+B1", "砖型+B2", "砖型+B1+B2"]:
            trade_df, portfolio_df = run_group(feature_map, group_name, candidate.column)
            rows.append(summarize(group_name, trade_df, portfolio_df, candidate.name))
            print(f"进度: {candidate.name} / {group_name}")

    result_df = pd.DataFrame(rows)
    result_df.to_csv(os.path.join(OUTPUT_DIR, "comparison.csv"), index=False, encoding="utf-8-sig")

    # 选融合里最优的 B1 变体
    fusion_only = result_df[result_df["group_name"] == "砖型+B1"].copy()
    fusion_only["drawdown_abs"] = fusion_only["max_drawdown"].abs()
    best_b1 = fusion_only.sort_values(
        ["annual_return", "drawdown_abs", "avg_trade_return", "success_rate"],
        ascending=[False, True, False, False],
    ).iloc[0]
    best_variant = best_b1["b1_variant"]
    final_view = result_df[result_df["b1_variant"] == best_variant].copy().sort_values(["annual_return", "max_drawdown"], ascending=[False, False])
    ...
    summary = {
        ...
        "b1_variants_tested": [c.name for c in candidates],
        "best_b1_variant": best_variant,
        "best_b1_row": best_b1.to_dict(),
        "best_variant_group_comparison": final_view.to_dict(orient="records"),
    }
