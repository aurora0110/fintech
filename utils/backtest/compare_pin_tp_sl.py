...
OUTPUT_DIR = "/Users/lidongyang/Desktop/Qstrategy/results/pin_tp_sl_comparison"
MIN_BARS = 160
...
def stop_price(signal_low: float, stop_mode: str) -> float | None:
    if not np.isfinite(signal_low) or signal_low <= 0:
        return None
    if stop_mode == "none":
        return None
    if stop_mode == "entry_low":
        return signal_low
    if stop_mode == "entry_low_x_0.99":
        return signal_low * 0.99
    return None


def simulate_trade(df: pd.DataFrame, signal_idx: int, tp_pct: float | None, stop_mode: str) -> Optional[dict]:
    ...
    scheduled_exit_idx = signal_idx + 1 + HOLD_DAYS
    if scheduled_exit_idx >= len(df):
        return None

    signal_low = float(df.at[signal_idx, "low"])
    sl_price = stop_price(signal_low, stop_mode)
        ...
        if sl_price is not None and float(df.at[j, "low"]) <= sl_price:
            ...
            if next_idx > entry_idx:
                ...
                if np.isfinite(px) and px > 0:
                    ...
                    break
        if tp_price is not None and float(df.at[j, "high"]) >= tp_price:
            next_idx = min(j + 1, len(df) - 1)
            if next_idx > entry_idx:
                px = float(df.at[next_idx, "open"])
                if np.isfinite(px) and px > 0:
...
def build_trade_df(feature_map: Dict[str, pd.DataFrame], tp_pct: float | None, stop_mode: str) -> pd.DataFrame:
    trades: List[dict] = []
    for code, df in feature_map.items():
        for signal_idx in np.flatnonzero(df["signal_mask"].to_numpy()):
            trade = simulate_trade(df, int(signal_idx), tp_pct, stop_mode)
...
def summarize(name: str, trade_df: pd.DataFrame, portfolio_df: pd.DataFrame, tp_pct: float | None, stop_mode: str) -> dict:
...
def main() -> None:
    ...
    configs = [
        ...
        ("tp_0.03", 0.03, "none"),
        ("tp_0.05", 0.05, "none"),
        ("sl_entry_low", None, "entry_low"),
        ("sl_entry_low_x_0.99", None, "entry_low_x_0.99"),
        ("tp_0.03__sl_entry_low", 0.03, "entry_low"),
        ("tp_0.03__sl_entry_low_x_0.99", 0.03, "entry_low_x_0.99"),
        ("tp_0.05__sl_entry_low", 0.05, "entry_low"),
        ("tp_0.05__sl_entry_low_x_0.99", 0.05, "entry_low_x_0.99"),
    ]
    rows = []
    for name, tp_pct, stop_mode in configs:
        trade_df = build_trade_df(feature_map, tp_pct, stop_mode)
        portfolio_df = build_portfolio_curve(trade_df)
        rows.append(summarize(name, trade_df, portfolio_df, tp_pct, stop_mode))
        print(f"策略完成: {name}")
    ...
    summary = {"hold_days": HOLD_DAYS, "results": result_df.to_dict(orient="records")}
