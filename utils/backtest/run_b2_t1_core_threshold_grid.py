...
OUTPUT_DIR = os.environ.get(
    "B2_T1_CORE_THRESHOLD_OUTPUT_DIR",
    "/Users/lidongyang/Desktop/Qstrategy/results/b2_t1_core_threshold_grid",
)
...
class Combo:
    ...
    def combo_name(self) -> str:
        return (
            f"涨幅下限_{self.当日涨幅下限:.2f}"
            f"__上影线系数_{self.上影线系数:.1f}"
            f"__J上限_{self.J上限}"
        )
...
def build_combos() -> List[Combo]:
    return [
        Combo(当日涨幅下限=gain_floor, 上影线系数=shadow_coef, J上限=j_cap)
        for gain_floor, shadow_coef, j_cap in product([0.03, 0.04, 0.05], [0.2, 0.3, 0.5], [70, 80, 90])
    ]
    ...
    for combo in combos:
        ...
        for code, df in feature_map.items():
            mask = (
                ...
                & df["J启动骨架"]
                & df["双线附近启动"]
                & (df["量比_五日"].between(1.5, 2.5, inclusive="both"))
                & (df["收盘位置"] >= 0.7)
                & (df["距20日高点"] >= -0.05)
            )
            mask &= df["涨幅"] >= combo.当日涨幅下限
            mask &= (df["实体长度"] <= EPS) | (df["上影线长度"] <= df["实体长度"] * combo.上影线系数 + EPS)
            mask &= df["J"] < combo.J上限
