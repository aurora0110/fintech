    ...
    return [
        {
            "策略": "fixed_days",
            "变体名称": "固定持有10天",
            "参数": dict(common | {"固定持有天数": 10, "结构退出最长持有天数": 10}),
        },
        {
            "策略": "fixed_days",
            "变体名称": "固定持有15天",
            "参数": dict(common | {"固定持有天数": 15, "结构退出最长持有天数": 15}),
        },
        {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="修复型候选池2.0验证实验")
    parser.add_argument("--data-dir", default="/Users/lidongyang/Desktop/Qstrategy/data/20260226")
    parser.add_argument("--output-root", default="/Users/lidongyang/Desktop/Qstrategy/results/repair_pool_v2_experiment_v1")
    ...
    result_path = output_root / "修复候选池2.0结果.csv"
    ...
    with result_path.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writeheader()
        for idx, task in enumerate(tqdm(tasks, desc="修复候选池2.0实验", unit="组"), start=1):
            _写状态(status_path, {"阶段": "运行修复候选池2.0实验", "完成组数": idx - 1, "总组数": len(tasks), "评分法": task["评分法"], "策略": task["策略"], "变体名称": task["变体名称"]})
