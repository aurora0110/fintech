...
def _退出配置(
    *,
    ...
    return {
        ...
        "启动失败观察天数": 启动失败观察天数,
        "启动失败最小涨幅": 启动失败最小涨幅,
        "第二层启动失败观察天数": 第二层启动失败观察天数,
        "第二层启动失败最小涨幅": 第二层启动失败最小涨幅,
        "保本触发涨幅": 0.05,
        "保本止损系数": 1.0,
        "强票延长持有启用": 强票延长持有启用,
        "强票延长额外天数": 强票延长额外天数,
...
def _方案列表() -> list[dict]:
    return [
        ...
        },
        {
            "变体名称": "持仓集中_6只",
            "候选配置": _候选配置(),
            "退出配置": _退出配置(名称="固定持有40_趋势线偏离_集中6"),
            "组合配置": {"最大持仓数": 6},
        },
        {
            "变体名称": "持仓集中_4只",
            "候选配置": _候选配置(),
            "退出配置": _退出配置(名称="固定持有40_趋势线偏离_集中4"),
            "组合配置": {"最大持仓数": 4},
        },
        {
            "变体名称": "强票延长20天",
        ...
        {
            "变体名称": "利润保护改历史启动特征_支撑3",
            "候选配置": _候选配置(),
            "退出配置": _退出配置(
                名称="固定持有40_历史启动特征_支撑3",
                利润保护模式="历史启动特征",
            ),
            "组合配置": {"最大持仓数": 10},
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(description="围绕新主基线3.0做收益率提升实验")
    ...
    parser.add_argument("--output-root", default="/Users/lidongyang/Desktop/Qstrategy/results/trend_yield_boost_v1")
    ...
    with result_path.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writeheader()
        for idx, variant in enumerate(tqdm(variants, desc="收益率提升实验", unit="组"), start=1):
            _写状态(
                status_path,
                {
                    "阶段": "运行收益率提升实验",
    ...
    rows_sorted = sorted(rows, key=_综合评分, reverse=True)
    summary = {
        "总组数": len(rows),
        "综合最优": rows_sorted[0] if rows_sorted else None,
        "收益最优": max(rows, key=lambda r: float(r["年化收益"])) if rows else None,
        "回撤最优": min(rows, key=lambda r: float(r["最大回撤"])) if rows else None,
        "全部结果": rows_sorted,
    }
