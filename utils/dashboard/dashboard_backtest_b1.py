...
# 设置页面配置
st.set_page_config(
    page_title="股票交易策略可视化",
    page_icon="📈",
    layout="wide"
)

# 主标题
st.title("股票交易策略可视化工具")

# 侧边栏
st.sidebar.header("配置选项")

# 固定读取指定的JSON文件
target_file = "backtest_b1_strategy_20260221.json"
...
try:
    with open(file_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    st.sidebar.success(f"成功加载文件: {target_file}")
...
overview_cols[0].metric("总收益率", f"{data['total_return'] * 100:.2f}%")
overview_cols[1].metric("年化收益率", f"{data['annual_return'] * 100:.2f}%")
overview_cols[2].metric("交易成功率", f"{data['success_rate'] * 100:.2f}%")
overview_cols[3].metric("总交易次数", f"{data['total_trades']}")

# 显示收益率最高的前10只股票
st.header("收益率最高的前10只股票")
top_10_df = pd.DataFrame(data['top_10_stocks'])
...
# 输入股票代码
st.header("股票交易详情")
stock_code = st.text_input("输入股票代码", "688981")

if stock_code not in data['stock_results']:
...
stats_cols[0].metric("交易次数", f"{stock_data['num_trades']}")
stats_cols[1].metric("总收益", f"{stock_data['total_profit']:.2f}")
stats_cols[2].metric("总收益率", f"{stock_data['total_return'] * 100:.2f}%")

# 加载股票历史数据
data_dir = "/Users/lidongyang/Desktop/Qstrategy/data/backtest_data"
target_stock_file = None
# 直接搜索包含股票代码的文件，而不是列出所有文件
...
fig = make_subplots(rows=3, cols=1, shared_xaxes=True, 
                   ...
                   subplot_titles=(f'{stock_code} K线图', 'KDJ指标', '成交量'),
...
fig.add_trace(
    go.Candlestick(
        ...
        name='K线',
        increasing=dict(line=dict(color='red')),  # 阳线红色
        decreasing=dict(line=dict(color='green'))  # 阴线绿色
    ), row=1, col=1)
...
if buy_signals:
    ...
    buy_prices = [b['price'] * (1 - b['offset'] * 0.005) for b in buy_data]  # 略微错开价格
...
if sell_signals:
    ...
    sell_prices = [s['price'] * (1 + s['offset'] * 0.005) for s in sell_data]  # 略微错开价格
...
# 显示交易记录
st.subheader("交易记录")
trades_df = pd.DataFrame(stock_data['trades'])
